...
export interface FailedMemberData {
    ...
    utilizationRatio: number;  // > 1.0 means failure
...
export type FailureMode =
    ...
    | 'combined_stress'
    | 'deflection_exceeded'
    | 'slenderness_exceeded';

export interface SectionData {
    name: string;           // e.g., "W12x26", "ISMB 300"
    area: number;           // mm² or in²
    momentOfInertia: number; // mm⁴ or in⁴
    sectionModulus: number;  // mm³ or in³
    radiusOfGyration: number; // mm or in
    flangeWidth: number;    // mm or in
    webThickness: number;   // mm or in
}

export interface GeometryData {
    length: number;         // meters or feet
    effectiveLength: number; // Kl value
    unsupportedLength: number; // For LTB checks
    endConditions: 'fixed-fixed' | 'fixed-pinned' | 'pinned-pinned' | 'cantilever';
}

export interface LoadingData {
    axialForce: number;     // kN or kips (compression positive)
    shearForce: number;     // kN or kips
    bendingMoment: number;  // kN-m or kip-ft
    loadCombination: string; // e.g., "1.2DL + 1.6LL"
}
...
export interface AIFixSuggestion {
    ...
    estimatedImprovement: string; // e.g., "Reduces ratio by ~20%"
...
export class EngineeringCopilotService {
    ...
    async analyzeFailedMember(memberData: FailedMemberData): Promise<CopilotResponse> {
        ...
        // Generate AI response (mock for now, replace with actual API call)
     ...
     */
    private constructAnalysisPrompt(data: FailedMemberData): string {
...
- Area: ${data.section.area.toFixed(0)} mm²
...
`;
     ...
     */
    private async generateAIResponse(data: FailedMemberData): Promise<CopilotResponse> {
        // Simulate API delay
        await new Promise(resolve => setTimeout(resolve, 500));

        // Generate contextual suggestions based on failure mode
     ...
     */
    private getFixSuggestions(data: FailedMemberData): AIFixSuggestion[] {
     ...
     */
    private getAnalysisText(data: FailedMemberData): string {
        const { failureMode, utilizationRatio, section, geometry } = data;
        const slenderness = (geometry.effectiveLength * 1000) / section.radiusOfGyration;
...
Review the suggestions below for practical fixing options.`;
     ...
     */
    private getFailureModeDescription(mode: FailureMode): string {
     ...
     */
    private suggestLargerSection(current: string): string {
     ...
     */
    private suggestWiderFlangeSection(current: string): string {
        const upgrades: Record<string, string> = {
     ...
     */
    private suggestDeeperSection(current: string): string {
        const upgrades: Record<string, string> = {
            'W12x26': 'W14x26',
            'W14x30': 'W16x31',
            'W16x36': 'W18x35',
     ...
     */
    private getAdditionalNotes(data: FailedMemberData): string {
        const notes: string[] = [];
        ...
        if (data.geometry.length > 8) {
            notes.push('📏 Long member: Check lateral bracing requirements carefully.');
        }

        if (data.loading.axialForce > 0 && data.loading.bendingMoment > 0) {
            notes.push('🔄 Combined loading: Ensure interaction equation (H1) compliance after fixes.');
        }
     ...
     */
    private getCodeReference(mode: FailureMode): string {
        const references: Record<FailureMode, string> = {
...
export const engineeringCopilot = new EngineeringCopilotService('is800');
