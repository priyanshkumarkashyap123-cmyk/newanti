  ...
  try {
    ...
    if (!email || !password || !firstName || !lastName) {
      res.status(400).json({
        ...
        errors: {
          email: !email ? "Email is required" : void 0,
          password: !password ? "Password is required" : void 0,
          firstName: !firstName ? "First name is required" : void 0,
          lastName: !lastName ? "Last name is required" : void 0
        }
      });
      return;
    }
...
router.post("/signin", async (req, res) => {
  try {
    ...
    await RefreshTokenModel.create({
      userId: user._id,
      token: refreshToken,
      expiresAt: new Date(Date.now() + (rememberMe ? 30 : 7) * 24 * 60 * 60 * 1e3)
    });
    await UserModel.updateOne(
      { _id: user._id },
