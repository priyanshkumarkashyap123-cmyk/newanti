...
async function runPythonAnalysis(scriptCode, inputData) {
...
router.post("/pdelta", async (req, res) => {
  try {
...
router.post("/spectrum", async (req, res) => {
  try {
    const request = req.body;
    const pythonScript = `
...
router.post("/buckling", async (req, res) => {
  try {
    const request = req.body;
    const pythonScript = `
