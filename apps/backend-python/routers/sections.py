"""
Section Design, Material & Plate Element Endpoints
"""
    ...
    try:
        ...
        if request.member_type.lower() == "beam":
            sections = await asyncio.to_thread(lambda: recommender.recommend_for_beam(
                required_Mx=request.required_Mx, required_My=request.required_My,
                length=request.length, section_type=request.section_type,
            ...
            sections = await asyncio.to_thread(lambda: recommender.recommend_for_column(
                required_P=request.required_P, length=request.length,
                section_type=request.section_type, safety_factor=request.safety_factor
            ))
        else:
        ...
        for section in sections:
            ...
            recommendations.append({
                ...
                "properties": {
                    "area": section.area, "depth": section.depth, "width": section.width,
                    "tw": section.tw, "tf": section.tf, "ixx": section.ixx, "iyy": section.iyy,
                    "zxx": section.zxx, "zyy": section.zyy, "rxx": section.rxx, "ryy": section.ryy,
                    "weight_per_meter": section.weight_per_meter
                },
                "capacity": capacity,
                "material": {"fy": section.fy, "fu": section.fu, "E": section.E}
            })
        ...
        return {
            ...
            "criteria": {
                "member_type": request.member_type, "required_Mx": request.required_Mx,
                "required_My": request.required_My, "required_P": request.required_P,
                "length": request.length, "section_type": request.section_type,
                "safety_factor": request.safety_factor
            }
        }
    ...
    try:
        ...
        if shape_type in ("i_beam", "ibeam"):
            section = StandardShapes.i_beam(
                depth=dims['depth'], width=dims['width'],
                web_thick=dims['web_thickness'], flange_thick=dims['flange_thickness'],
            ...
            section = StandardShapes.angle(
                leg1=dims['leg1'], leg2=dims['leg2'], thickness=dims['thickness'],
            ...
            section = StandardShapes.circular(
                diameter=dims['diameter'], segments=int(dims.get('segments', 32)),
            ...
            section = StandardShapes.built_up_i(
                depth=dims['depth'], top_width=dims['top_width'], bot_width=dims['bot_width'],
                web_thick=dims['web_thickness'], top_thick=dims['top_thickness'],
            ...
            section = StandardShapes.composite_beam(
                depth=dims['depth'], width=dims['width'],
                web_thick=dims['web_thickness'], flange_thick=dims['flange_thickness'],
                slab_width=dims['slab_width'], slab_thick=dims['slab_thickness'],
            ...
            section = StandardShapes.lipped_channel(
                depth=dims['depth'], width=dims['width'],
                thickness=dims['thickness'], lip=dims['lip'],
...
async def create_plate(request: CreatePlateRequest, req: Request):
    ...
    try:
        ...
        material = req.app.state.materials.get(request.material_id)

        if material is None:
            if request.material_id:
                print(f"Warning: Material ID {request.material_id} not found, using default steel.")
            from analysis.material_models import ElasticPlasticSteel
            material = ElasticPlasticSteel(fy=250.0)

        plate = PlateElement(node_ids=request.node_ids, thickness=request.thickness, material=material)

        if not hasattr(req.app.state, "plates"):
            req.app.state.plates = {}

        plate_id = f"plate_{len(req.app.state.plates) + 1}"
        req.app.state.plates[plate_id] = plate
