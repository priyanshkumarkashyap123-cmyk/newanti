...
class LayoutV2Response(BaseModel):
    ...
    space_syntax: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Space-syntax graph analysis: integration, control, depth histogram, planarity",
    )
    structural_handoff: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Structural engineering handoff: wall stacking, cantilevers, slab panels",
    )
    mep_schedule: Optional[Dict[str, Any]] = Field(
        default=None,
        description="MEP schedule: plumbing stacks, electrical circuits, HVAC loads",
    )
...
# =====================================================================
# GA OPTIMIZATION ENDPOINT  (Phase D)
# =====================================================================
...
class GAOptimizeRequest(BaseModel):
    """Request for GA-based population optimization."""
    site: SiteRequest
    global_constraints: Optional[GlobalConstraintsRequest] = None
    nodes: List[RoomNodeRequest] = Field(..., min_length=1, max_length=200)
    adjacency_matrix: List[AdjacencyMatrixEntry] = Field(default_factory=list)
...
class GAOptimizeResponse(BaseModel):
    """Response from GA-based optimization."""
    success: bool
    best_fitness: float
...
async def ga_optimize_layout(request: GAOptimizeRequest):
    """Population-based optimization using a Genetic Algorithm.

    Seeds the initial population from multiple BSP solver runs, then
    evolves through selection, crossover, and mutation to explore a
    wider solution space than single-run BSP+SA.
    """
    try:
        _validate_v2(request)
        ...
        # Convert best chromosome to solver solution for reporting
        rooms_by_id = {r.id: r for r in room_nodes}
        best_placements = result.best_chromosome.to_placements(rooms_by_id)

        # Build a temporary solver to generate the full report from best placements
        solver = LayoutSolverV2(
            site=site_cfg,
            constraints=constraints,
            rooms=room_nodes,
        ...
        # Inject the GA-optimized solution
        ...
        compliance = [
            ComplianceItemResponse(**ci) for ci in report.get("compliance_items", [])
        ]
        ...
        raise HTTPException(
            status_code=500, detail=f"GA optimization failed: {exc}"
        ) from exc
