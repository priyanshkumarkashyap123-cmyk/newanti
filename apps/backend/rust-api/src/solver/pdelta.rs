    ...
    fn compute_axial_forces(
        ...
        member_geometry
            ...
            .map(|member| {
                ...
                let u_i_y = if member.node_i_dof + 1 < n_dof {
                    displacements[member.node_i_dof + 1]
                } else {
                    0.0
                };
                let u_i_z = if member.node_i_dof + 2 < n_dof {
                    displacements[member.node_i_dof + 2]
                } else {
                    0.0
                };

                let u_j_x = displacements[member.node_j_dof];
                let u_j_y = if member.node_j_dof + 1 < n_dof {
                    displacements[member.node_j_dof + 1]
                } else {
                    0.0
                };
                let u_j_z = if member.node_j_dof + 2 < n_dof {
                    displacements[member.node_j_dof + 2]
                } else {
                    0.0
                };
