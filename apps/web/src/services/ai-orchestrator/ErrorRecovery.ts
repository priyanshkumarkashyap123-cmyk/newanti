...
export class AIErrorClassifier {
   ...
   */
  classify(error: Error, provider: AIProviderType, requestType: AIRequestType): ClassifiedError {
    ...
    if (this.isSafetyError(msg)) {
      return {
        ...
        retryable: false,
        suggestedAction: 'simplify-request',
        userMessage: 'Request was filtered by safety checks. Please rephrase your query.',
  ...
  private isNetworkError(msg: string): boolean {
    return ['econnreset', 'econnrefused', 'enotfound', 'enetunreach', 'fetch failed', 'network', 'dns', 'failed to fetch'].some(p => msg.includes(p));
  }

  private isAuthError(msg: string): boolean {
    return ['401', '403', 'unauthorized', 'forbidden', 'invalid.*api.*key', 'invalid.*token', 'authentication'].some(p => new RegExp(p).test(msg));
  }

  private isRateLimitError(msg: string): boolean {
    return ['429', 'rate.?limit', 'quota.?exceeded', 'too many requests', 'throttl'].some(p => new RegExp(p).test(msg));
  }

  private isBudgetError(msg: string): boolean {
    return ['budget', 'token.*limit.*exceeded', 'cost.*exceeded', 'insufficient.*funds', 'billing'].some(p => new RegExp(p).test(msg));
  }

  private isTimeoutError(msg: string): boolean {
    return ['timeout', 'etimedout', 'deadline', 'timed out', 'too long'].some(p => msg.includes(p));
  }

  private isSafetyError(msg: string): boolean {
    return ['safety', 'content.?filter', 'blocked', 'harmful', 'policy'].some(p => new RegExp(p).test(msg));
  }

  private isProviderError(msg: string): boolean {
    return ['500', '502', '503', '504', 'internal server error', 'bad gateway', 'service unavailable'].some(p => msg.includes(p));
  }

  private isCapacityError(msg: string): boolean {
    return ['capacity', 'overloaded', 'server busy', 'resource exhausted'].some(p => msg.includes(p));
  }

  private isInvalidOutputError(msg: string): boolean {
    return ['invalid json', 'parse error', 'unexpected token', 'malformed', 'empty response', 'null response'].some(p => msg.includes(p));
  }
}
...
export class AIErrorRecoveryEngine {
  ...
  private localFallbacks: Map<AIRequestType, (request: AIRequest) => Promise<Partial<AIResponse>>>;
  ...
  async recover(
  ...
  ): Promise<{ response: AIResponse | null; classified: ClassifiedError; recoveryPath: string[] }> {
    ...
    const shouldRetry = classified.suggestedAction === 'retry' || classified.suggestedAction === 'retry-with-backoff';
    const shouldSwitchProvider = shouldRetry || classified.suggestedAction === 'switch-provider';
    const shouldUseLocalAI = shouldSwitchProvider || classified.suggestedAction === 'use-local-ai';
      ...
      for (const alt of alternatives) {
        ...
        try {
          ...
          this.eventBus.emit({
            ...
            timestamp: new Date(),
            data: { originalProvider: provider, fallbackProvider: alt, success: true },
          });
          return { response, classified, recoveryPath };
        } catch (altError) {
          recoveryPath.push(`${alt} failed: ${altError instanceof Error ? altError.message : 'unknown'}`);
        }
      }
      ...
      if (localFallback) {
        try {
          ...
          const response: AIResponse = {
            ...
            content: partialResponse.content || 'Generated using local AI engine (offline mode)',
            confidence: partialResponse.confidence ?? 0.5,
            usage: { inputTokens: 0, outputTokens: 0, totalTokens: 0, estimatedCostUSD: 0 },
            latency: 0,
            cached: false,
            warnings: ['Generated by local AI engine (reduced capability)', `Original error: ${classified.userMessage}`],
            errors: [],
            timestamp: new Date(),
            structuredData: partialResponse.structuredData,
            fallbackChain: [
              { provider, model: '', status: 'failed', error: classified.category },
              { provider: 'local', model: 'rule-engine', status: 'success' },
            ],
          };
          return { response, classified, recoveryPath };
        } catch (localError) {
          recoveryPath.push(`Local fallback failed: ${localError instanceof Error ? localError.message : 'unknown'}`);
        }
      }
    }
   ...
   */
  registerFallback(type: AIRequestType, handler: (request: AIRequest) => Promise<Partial<AIResponse>>): void {
  ...
  private registerDefaultFallbacks(): void {
    ...
    this.registerFallback('structure-generation', async (request) => {
      ...
      return {
        content: template ? 'Structure generated using template matching (offline)' : 'Unable to generate structure offline',
        confidence: template ? 0.6 : 0.1,
        structuredData: template ? { type: 'structure', data: template } : undefined,
      };
    });
  ...
  private generateFromTemplate(request: AIRequest): AIResponse | null {
    ...
    const templates: Record<string, string> = {
      'simple beam': 'A simply supported beam has been generated with default parameters.',
  ...
  private generateLocalChatResponse(prompt: string): string {
    ...
    if (p.includes('what is') || p.includes('explain') || p.includes('describe')) {
  ...
  private matchStructureTemplate(prompt: string): NormalizedStructureData | null {
    const p = prompt.toLowerCase();
    ...
    if (p.includes('simple') && p.includes('beam')) {
      return {
        ...
        members: [
          { id: 'M1', startNodeId: 'N1', endNodeId: 'N2', type: 'beam', sectionId: 'ISMB 300' },
        ],
        loads: [
          { id: 'L1', type: 'distributed', targetType: 'member', targetId: 'M1', values: [-10], loadCase: 'Dead+Live' },
        ],
        supports: [
          { nodeId: 'N1', type: 'pinned', restraints: { dx: true, dy: true, dz: true, rx: false, ry: false, rz: false } },
          { nodeId: 'N2', type: 'roller', restraints: { dx: false, dy: true, dz: false, rx: false, ry: false, rz: false } },
        ],
        materials: [{ id: 'steel', name: 'Structural Steel E250', type: 'steel', E: 200e9, fy: 250e6 }],
        sections: [{ id: 'ISMB300', name: 'ISMB 300', type: 'I', area: 5.87e-3, Ixx: 8.603e-5, Iyy: 4.539e-6 }],
        metadata: { generatedBy: 'local', generatedAt: new Date(), confidence: 0.7, validationPassed: true, warnings: ['Template-generated'], structureDescription: 'Simply supported beam - 6m span' },
      };
    }
    ...
    if (p.includes('cantilever')) {
      return {
        ...
        members: [
          { id: 'M1', startNodeId: 'N1', endNodeId: 'N2', type: 'beam', sectionId: 'ISMB 250' },
        ],
        loads: [
          { id: 'L1', type: 'point', targetType: 'node', targetId: 'N2', values: [0, -20, 0], loadCase: 'Live' },
        ],
        supports: [
          { nodeId: 'N1', type: 'fixed', restraints: { dx: true, dy: true, dz: true, rx: true, ry: true, rz: true } },
        ],
        materials: [{ id: 'steel', name: 'Structural Steel E250', type: 'steel', E: 200e9, fy: 250e6 }],
        sections: [{ id: 'ISMB250', name: 'ISMB 250', type: 'I', area: 4.755e-3, Ixx: 5.132e-5, Iyy: 3.345e-6 }],
        metadata: { generatedBy: 'local', generatedAt: new Date(), confidence: 0.7, validationPassed: true, warnings: ['Template-generated'], structureDescription: 'Cantilever beam - 3m' },
      };
    }
    ...
    if (p.includes('portal') && p.includes('frame')) {
      return {
        ...
        members: [
          { id: 'C1', startNodeId: 'N1', endNodeId: 'N2', type: 'column', sectionId: 'ISMB 300' },
          { id: 'B1', startNodeId: 'N2', endNodeId: 'N3', type: 'beam', sectionId: 'ISMB 350' },
          { id: 'C2', startNodeId: 'N4', endNodeId: 'N3', type: 'column', sectionId: 'ISMB 300' },
        ],
        loads: [
          { id: 'L1', type: 'distributed', targetType: 'member', targetId: 'B1', values: [-15], loadCase: 'Dead+Live' },
        ],
        supports: [
          { nodeId: 'N1', type: 'fixed', restraints: { dx: true, dy: true, dz: true, rx: true, ry: true, rz: true } },
          { nodeId: 'N4', type: 'fixed', restraints: { dx: true, dy: true, dz: true, rx: true, ry: true, rz: true } },
        ],
        materials: [{ id: 'steel', name: 'Structural Steel E250', type: 'steel', E: 200e9, fy: 250e6 }],
        sections: [
          { id: 'ISMB300', name: 'ISMB 300', type: 'I', area: 5.87e-3, Ixx: 8.603e-5, Iyy: 4.539e-6 },
          { id: 'ISMB350', name: 'ISMB 350', type: 'I', area: 6.67e-3, Ixx: 1.366e-4, Iyy: 5.376e-6 },
        ],
        metadata: { generatedBy: 'local', generatedAt: new Date(), confidence: 0.65, validationPassed: true, warnings: ['Template-generated'], structureDescription: 'Portal frame - 8m span, 4m height' },
      };
    }
