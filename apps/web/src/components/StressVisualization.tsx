    ...
    <div className="fixed bottom-4 left-4 right-4 bg-[#131b2e] rounded-lg shadow-2xl border border-[#1a2333] max-h-[600px] overflow-y-auto z-50 pointer-events-auto" style={{ contain: 'layout' }}>
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="flex gap-2 pt-4 border-t border-[#1a2333]">
          ...
          <button type="button"
            onClick={() => {
              // Generate PDF-like stress report as printable HTML
              const reportHtml = `
<!DOCTYPE html>
<html>
<head>
  <title>Stress Analysis Report - BeamLab</title>
  <style>
...
<body>
  <h1>Stress Analysis Report</h1>
  <p>Generated: ${new Date().toLocaleString()}</p>
  <p>Members analyzed: ${results.length}</p>
  
  <h2>Member Stress Summary</h2>
  <table>
    <thead>
      <tr>
        <th>Member</th>
    ...
    <tbody>
      ${results.map(m => {
        const maxS = Math.max(...m.stress_points.map(p => p.von_mises));
        const minS = Math.min(...m.stress_points.map(p => p.sigma_x));
        return `<tr>
          <td style="text-align:left">${m.member_id}</td>
          <td>${maxS.toFixed(2)}</td>
          <td>${minS.toFixed(2)}</td>
          <td>${(m.check.max_utilization * 100).toFixed(1)}%</td>
          <td class="${m.check.passes ? 'pass' : 'fail'}">${m.check.passes ? 'PASS' : 'FAIL'}</td>
        </tr>`;
      }).join('')}
    </tbody>
  </table>
  ...
  <div class="summary">
    <strong>Summary:</strong> ${results.filter(m => m.check.passes).length}/${results.length} members pass stress checks.
    Max utilization: ${(Math.max(...results.map(m => m.check.max_utilization)) * 100).toFixed(1)}%
  </div>
  
  <p style="color:#94a3b8;font-size:11px;margin-top:40px;">Report generated by BeamLab Structural Analysis Platform</p>
</body>
</html>`;
              ...
              if (printWindow) {
                ...
                const link = document.createElement('a');
                link.href = url;
                link.download = `stress-report-${Date.now()}.html`;
                link.click();
                URL.revokeObjectURL(url);
              }
            }}
            className="flex-1 px-4 py-2.5 bg-gradient-to-r from-emerald-600 to-teal-600 text-white rounded-lg hover:from-emerald-700 hover:to-teal-700 transition-all font-semibold shadow-lg"
          >
            PDF Report
          </button>
        </div>
