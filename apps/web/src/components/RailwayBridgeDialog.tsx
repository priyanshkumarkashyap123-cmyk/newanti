...
import {
    X, Train, Calculator, ArrowDown, ArrowRight,
        ...
        <AnimatePresence>
            ...
            >
                <motion.div
                    ...
                    className="bg-zinc-900 rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden border border-zinc-700 shadow-2xl"
                ...
                >
                    ...
                    <div className="flex items-center justify-between p-4 border-b border-zinc-800">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-xl font-bold text-white">Railway Bridge Design</h2>
                                <p className="text-sm text-zinc-400">IRS Bridge Rules • MBG/HM Loading</p>
                            </div>
                        </div>
                    ...
                    <div className="p-6 overflow-y-auto max-h-[60vh]">
                        {activeTab === 'design' && (
                            <div className="space-y-6">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input
                                                ...
                                                className="w-full px-3 py-2 bg-zinc-700 rounded-lg text-white text-sm"
                                            />
                                        </div>
                                        <div>
                                        ...
                                        <div>
                                            <label className="block text-xs text-zinc-400 mb-1">Number of Panels</label>
                                            <input
                                                type="number"
                                        ...
                                        <div>
                                            <label className="block text-xs text-zinc-400 mb-1">Track Gauge (m)</label>
                                            <input
                                                type="number"
                                ...
                                <div className="bg-zinc-800/50 rounded-xl p-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        {(['MBG', 'HM', 'CooperE80'] as LoadingStandard[]).map((std) => (
                                            ...
                                            >
                                                <p className="text-sm font-medium text-white">{LOADING_STANDARDS[std].name}</p>
                                                <p className="text-xs text-zinc-400">{LOADING_STANDARDS[std].description}</p>
                                            </button>
                                        ))}
                                    </div>
                                </div>
                                ...
                                {/* Bridge Type */}
                                <div className="bg-zinc-800/50 rounded-xl p-4">
                                    <h3 className="text-sm font-semibold text-white mb-4">Deck Type</h3>
                                    <div className="flex gap-3">
                                        ...
                                        >
                                            <p className="text-sm font-medium text-white">Open Deck</p>
                                            <p className="text-xs text-zinc-400">Rails on sleepers</p>
                                        </button>
                                        <button
                                        ...
                                        >
                                            <p className="text-sm font-medium text-white">Ballasted Deck</p>
                                            <p className="text-xs text-zinc-400">Rails on ballast</p>
                                        </button>
                                    </div>
                                </div>
                            </div>
                        ...
                        {activeTab === 'loads' && (
                            <div className="space-y-4">
                                ...
                                <div className="bg-zinc-800/50 rounded-xl p-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="bg-zinc-900/50 p-3 rounded-lg">
                                            <p className="text-xs text-zinc-400">EUDL for BM</p>
                                            <p className="text-lg font-bold text-white">{eudl.bm.toFixed(1)} <span className="text-sm font-normal text-zinc-400">kN/m</span></p>
                                        </div>
                                        <div className="bg-zinc-900/50 p-3 rounded-lg">
                                            <p className="text-xs text-zinc-400">EUDL for SF</p>
                                            <p className="text-lg font-bold text-white">{eudl.sf.toFixed(1)} <span className="text-sm font-normal text-zinc-400">kN/m</span></p>
                                        </div>
                                        <div className="bg-zinc-900/50 p-3 rounded-lg">
                                            <p className="text-xs text-zinc-400">Impact Factor (CDA)</p>
                                            <p className="text-lg font-bold text-orange-400">{(cda * 100).toFixed(1)}%</p>
                                        </div>
                                        <div className="bg-zinc-900/50 p-3 rounded-lg">
                                            <p className="text-xs text-zinc-400">Factored Live Load</p>
                                            <p className="text-lg font-bold text-white">{factoredLoad.toFixed(1)} <span className="text-sm font-normal text-zinc-400">kN/m</span></p>
                                        </div>
                                        <div className="bg-zinc-900/50 p-3 rounded-lg">
                                            <p className="text-xs text-zinc-400">Longitudinal Force</p>
                                            <p className="text-lg font-bold text-white">{longForce.toFixed(1)} <span className="text-sm font-normal text-zinc-400">kN</span></p>
                                        </div>
                                        <div className="bg-zinc-900/50 p-3 rounded-lg">
                                            <p className="text-xs text-zinc-400">Racking Force</p>
                                            <p className="text-lg font-bold text-white">{rackingForce.toFixed(1)} <span className="text-sm font-normal text-zinc-400">kN</span></p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                {/* Axle Load Diagram */}
                                <div className="bg-zinc-800/50 rounded-xl p-4">
                                    <h3 className="text-sm font-semibold text-white mb-4">Axle Load Pattern ({config.loadingStandard})</h3>
                                        ...
                                        <div className="flex items-end gap-1 min-w-max">
                                            {LOADING_STANDARDS[config.loadingStandard].axleLoads.map((axle, i) => (
                                                <div key={i} className="flex flex-col items-center">
                                                    <span className="text-[10px] text-zinc-400 mb-1">{axle.load}kN</span>
                                                    <div
                                                        className={`w-3 ${axle.type === 'locomotive' ? 'bg-orange-500' : 'bg-blue-500'} rounded-t`}
                                                        style={{ height: `${axle.load / 5}px` }}
                                                    />
                                                    <div className="w-6 h-1 bg-zinc-600 rounded" />
                                                </div>
                                            ))}
                                        </div>
                                    ...
                                    <div className="flex gap-4 mt-3 text-xs">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-zinc-400">Wagon</span>
                                        </div>
                                    </div>
                                </div>
                        ...
                        {activeTab === 'results' && designResults && (
                            <div className="space-y-4">
                                ...
                                <div className={`rounded-xl p-4 flex items-center gap-3
                                    ...
                                    <div>
                                        <p className={`font-semibold ${designResults.passesDesign ? 'text-green-400' : 'text-red-400'}`}>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="bg-zinc-800/50 rounded-xl p-4">
                                        <p className="text-xs text-zinc-400">Max Axial Force</p>
                                        <p className="text-xl font-bold text-white">{(designResults.maxAxialForce / 1000).toFixed(1)} <span className="text-sm font-normal">MN</span></p>
                                    </div>
                                    <div className="bg-zinc-800/50 rounded-xl p-4">
                                        <p className="text-xs text-zinc-400">Max Bending Moment</p>
                                        <p className="text-xl font-bold text-white">{(designResults.maxBendingMoment / 1000).toFixed(1)} <span className="text-sm font-normal">MN·m</span></p>
                                    </div>
                                    <div className="bg-zinc-800/50 rounded-xl p-4">
                                        <p className="text-xs text-zinc-400">Max Deflection</p>
                                        <p className="text-xl font-bold text-white">{(designResults.maxDeflection * 1000).toFixed(1)} <span className="text-sm font-normal">mm</span></p>
                                        <p className="text-xs text-zinc-400">Limit: L/800 = {(config.span / 0.8).toFixed(1)} mm</p>
                                    </div>
                                    <div className="bg-zinc-800/50 rounded-xl p-4">
                                        <p className="text-xs text-zinc-400">Critical Member</p>
                                        <p className="text-xl font-bold text-orange-400">{designResults.criticalMember}</p>
                                        <p className="text-xs text-zinc-400">Top Chord at mid-span</p>
                                    </div>
                                </div>
                            </div>
                        )}
