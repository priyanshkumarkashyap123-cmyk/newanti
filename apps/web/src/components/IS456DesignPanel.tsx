...
export const IS456DesignPanel: FC<IS456DesignPanelProps> = ({
  ...
  if (!isPro) {
    return (
      <div className="p-6 bg-white dark:bg-zinc-900 rounded-xl text-center">
        <Crown className="w-12 h-12 mx-auto mb-4 text-yellow-500" />
        <h3 className="text-lg font-bold text-zinc-900 dark:text-white mb-2">Pro Feature</h3>
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 rounded-xl overflow-hidden">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-white/20 text-zinc-900 dark:text-white rounded-md text-sm font-medium hover:bg-white/30 disabled:opacity-50 border border-white/30"
      ...
      <div className="px-4 py-3 bg-zinc-100/60 dark:bg-zinc-800/60 border-b border-zinc-200 dark:border-zinc-700 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3 flex-1">
          <Sliders className="w-4 h-4 text-zinc-500 dark:text-zinc-400 shrink-0" />
          ...
          />
          <span className="text-sm font-mono text-zinc-900 dark:text-white min-w-[2.5rem] text-right">
            {extraFoS.toFixed(2)}
          </span>
        </div>
        ...
        <div className="border-b border-zinc-200 dark:border-zinc-700">
          ...
          <div className="max-h-48 overflow-y-auto divide-y divide-zinc-200 dark:divide-zinc-800">
            {Array.from(smartResults.entries()).map(([memberId, result]) => (
              <div
                key={memberId}
                className="px-4 py-2 flex items-center justify-between text-sm hover:bg-zinc-200/30 dark:hover:bg-zinc-800/30"
              >
                <span className="text-zinc-600 dark:text-zinc-300 font-medium">{memberId}</span>
      ...
      <div className="grid grid-cols-4 gap-4 p-4 border-b border-zinc-200 dark:border-zinc-800">
        <div className="text-center">
          <div className="text-2xl font-bold text-zinc-900 dark:text-white">{summary.total}</div>
        ...
        <div className="text-center">
          ...
          <div className="text-xs text-zinc-500 dark:text-zinc-400">Passing</div>
        </div>
        <div className="text-center">
        ...
        <div className="text-center">
          ...
          <div className="text-xs text-zinc-500 dark:text-zinc-400">Failing</div>
        </div>
      </div>
      ...
      <div className="max-h-96 overflow-y-auto">
        {designResults.map((result: any) => (
          ...
          >
            <summary className="flex items-center justify-between px-4 py-3 cursor-pointer hover:bg-zinc-200/50 dark:hover:bg-zinc-800/50">
              ...
              <div className="flex items-center gap-3">
                ...
                <ChevronDown className="w-4 h-4 text-zinc-500 dark:text-zinc-400 group-open:rotate-180 transition-transform" />
              </div>
            </summary>
            <div className="px-4 pb-4 space-y-2">
              {/* Rebar Details */}
              <div className="grid grid-cols-2 gap-4 mb-4 p-2 bg-zinc-100/50 dark:bg-zinc-800/50 rounded text-xs border border-zinc-200 dark:border-zinc-700">
                {Object.entries(result.details || {}).map(([key, value]) => (
                  <div key={key}>
                    <div className="text-zinc-500 dark:text-zinc-400 capitalize">
                      {key.replace("_", " ")}
                    </div>
                    <div className="text-zinc-900 dark:text-white font-mono">{String(value)}</div>
                  </div>
                ))}
              </div>
              ...
              {result.checks.map((check: any, i: number) => (
                <div
                  key={i}
                  className="flex items-center justify-between text-sm py-2 border-t border-zinc-800/50"
                >
                  <span className="text-zinc-500 dark:text-zinc-400">{check.name}</span>
                  <div className="flex items-center gap-4">
                    <span className="text-zinc-500 dark:text-zinc-400">
