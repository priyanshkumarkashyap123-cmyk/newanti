  ...
  return (
    <motion.div
      ...
      className={`flex items-center gap-3 p-3 rounded-xl transition-colors ${
        isActive ? 'bg-blue-500/10 border border-blue-500/30' : 'bg-zinc-800/30'
      }`}
    >
    ...
    <div className={`bg-zinc-950 rounded-2xl overflow-hidden ${className}`}>
      ...
      <div className="p-4 space-y-4">
          ...
          <div className="grid grid-cols-4 gap-2">
            {ANALYSIS_TYPES.slice(0, 4).map(({ type, name, icon, color }) => (
              ...
              >
                ...
                <span className={`text-xs ${config.type === type ? 'text-white' : 'text-zinc-400'}`}>
                  {name}
                </span>
              </button>
            ))}
                ...
                <div className="p-4 pt-0 grid grid-cols-2 gap-4 border-t border-zinc-800">
                  <div>
                    ...
                    <input
                      ...
                      disabled={status !== 'idle'}
                      className="w-full px-3 py-2 bg-zinc-800 border border-zinc-700 rounded-lg text-sm text-white font-mono disabled:opacity-50"
                    />
                  </div>
                  <div>
        ...
        <div className="grid grid-cols-3 gap-4">
          {/* Main Progress */}
          <div className="col-span-1 flex items-center justify-center p-4 bg-zinc-900/50 rounded-xl border border-zinc-800">
            <CircularProgress
              value={progress}
          ...
          {status === 'idle' ? (
            ...
            <>
              <motion.button
                ...
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                className="flex-1 flex items-center justify-center gap-2 py-4 bg-zinc-800 hover:bg-zinc-700 text-white font-semibold rounded-xl"
              >
