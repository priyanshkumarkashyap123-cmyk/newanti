...
const Sparkline: React.FC<{
  ...
  return (
    <svg width={width} height={height} className="overflow-hidden">
      ...
      <defs>
        <linearGradient
          id={`sparklineGradient-${color.replace("#", "")}`}
          x1="0%"
          y1="0%"
          x2="0%"
          y2="100%"
      ...
      <div className="space-y-2">
        {data.map((item, index) => (
          <div key={index} className="flex items-center gap-3">
            {showLabels && (
              <span className="text-xs text-zinc-500 dark:text-zinc-400 w-16 truncate">
                {item.label}
              </span>
            )}
            ...
            <span className="text-xs font-mono text-zinc-900 dark:text-white w-16 text-right">
      ...
      {data.map((item, index) => (
        <div key={index} className="flex flex-col items-center gap-1 flex-1">
          <motion.div
            ...
            initial={{ height: 0 }}
            animate={{
              height: `${(Math.abs(item.value) / max) * (height - 40)}px`,
            }}
            transition={{ duration: 0.5, delay: index * 0.1 }}
          />
          {showLabels && (
            <span className="text-xs text-zinc-500 dark:text-zinc-400 truncate max-w-full">
              {item.label}
            </span>
          )}
        </div>
  ...
  return (
    <motion.div
      ...
      className={`relative overflow-hidden rounded-2xl border p-4 bg-gradient-to-br ${
        status
          ? statusColors[status]
          : "from-zinc-100/50 dark:from-zinc-800/50 to-zinc-50/50 dark:to-zinc-900/50 border-zinc-200/50 dark:border-zinc-700/50"
      }`}
    >
      <div className="flex items-start justify-between mb-3">
        <div
          className={`p-2.5 rounded-xl ${status ? iconColors[status] : "bg-zinc-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400"}`}
        >
          {icon}
        </div>
        {sparklineData && (
...
const MemberUtilizationTable: React.FC<{
  ...
  return (
    <div className="bg-white/50 dark:bg-zinc-900/50 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
      {/* Header */}
      <div className="p-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="pl-9 pr-4 py-2 bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg text-sm text-zinc-900 dark:text-white placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-blue-500 w-48"
          />
        </div>
      </div>
            ...
            <tr>
              {[
                ...
                <th
                  ...
                  className="px-4 py-3 text-left text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider cursor-pointer hover:text-zinc-900 dark:hover:text-white transition-colors"
                >
          ...
          <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
            {sortedMembers.map((member, index) => (
              <motion.tr
                ...
                className={`hover:bg-zinc-200/50 dark:hover:bg-zinc-800/50 cursor-pointer transition-colors ${
              ...
              >
                ...
                <td className="px-4 py-3 text-sm font-mono text-zinc-600 dark:text-zinc-300">
                  {member.axialForce.toFixed(1)}
                </td>
                <td className="px-4 py-3 text-sm font-mono text-zinc-600 dark:text-zinc-300">
                  {member.momentY.toFixed(1)}
                </td>
                <td className="px-4 py-3 text-sm font-mono text-zinc-600 dark:text-zinc-300">
                  {member.maxStress.toFixed(1)}
                </td>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-mono text-zinc-900 dark:text-white w-12">
                      {(member.utilization * 100).toFixed(0)}%
                    </span>
                  </div>
                </td>
                <td className="px-4 py-3">{getStatusIcon(member.status)}</td>
              </motion.tr>
            ))}
          </tbody>
        </table>
...
const ModalAnalysisChart: React.FC<{
  ...
  return (
    <div className="bg-white/50 dark:bg-zinc-900/50 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-4">
      ...
      {/* Frequency Chart */}
      <div className="mb-6">
        <p className="text-xs text-zinc-500 dark:text-zinc-400 mb-2">Natural Frequencies (Hz)</p>
          ...
          {modes.slice(0, 8).map((mode, i) => (
            ...
            >
              <motion.div
                ...
                initial={{ height: 0 }}
                animate={{
                  height: `${(mode.frequency / Math.max(...modes.map((m) => m.frequency))) * 100}px`,
                }}
                transition={{ duration: 0.5, delay: i * 0.1 }}
              />
              <span className="text-xs text-zinc-500 dark:text-zinc-400">M{mode.mode}</span>
            </motion.div>
          ))}
        </div>
      </div>
          ...
          {["X", "Y", "Z"].map((dir, i) => {
            ...
            return (
              <div key={dir} className="flex items-center gap-3">
                <span className="text-xs text-zinc-500 dark:text-zinc-400 w-8">{dir}-Dir</span>
                ...
                <span
                  className={`text-xs font-mono w-12 text-right ${finalValue >= 0.9 ? "text-emerald-400" : "text-amber-400"}`}
                >
        ...
        {selectedMode !== null && (
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-4 text-xs">
              ...
              <div>
                <span className="text-zinc-500 dark:text-zinc-400">Max Participation</span>
                <p className="text-zinc-900 dark:text-white font-mono">
                  {(
                    Math.max(
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-zinc-900 dark:text-white">{combo.name}</span>
              </div>
            ...
            <div className="grid grid-cols-3 gap-4 text-xs">
              <div>
                <span className="text-zinc-500 dark:text-zinc-400">Max Disp.</span>
                <p className="text-zinc-900 dark:text-white font-mono">
...
const ForceDiagram: React.FC<{
  ...
  return (
    <svg width={width} height={height} className="overflow-hidden">
      ...
      {/* Filled area */}
      <path d={areaD} fill={`url(#diagramGradient-${type})`} opacity="0.4" />
...
export const InteractiveResultsDashboard: React.FC<{
  ...
  const [modes] = useState<ModalResult[]>([
    {
      ...
      frequency: 2.45,
      period: 0.408,
      massParticipationX: 0.72,
      massParticipationY: 0.05,
      massParticipationZ: 0.02,
      cumulativeX: 0.72,
      cumulativeY: 0.05,
      cumulativeZ: 0.02,
    },
    {
      ...
      frequency: 4.82,
      period: 0.207,
      massParticipationX: 0.08,
      massParticipationY: 0.68,
      massParticipationZ: 0.03,
      cumulativeX: 0.8,
      cumulativeY: 0.73,
      cumulativeZ: 0.05,
    },
    {
      ...
      frequency: 7.15,
      period: 0.14,
      massParticipationX: 0.05,
      massParticipationY: 0.12,
      massParticipationZ: 0.65,
      cumulativeX: 0.85,
      cumulativeY: 0.85,
      cumulativeZ: 0.7,
    },
    ...
    },
    {
      ...
      frequency: 12.45,
      period: 0.08,
      massParticipationX: 0.04,
      massParticipationY: 0.06,
      massParticipationZ: 0.1,
      cumulativeX: 0.97,
      cumulativeY: 0.96,
      cumulativeZ: 0.95,
    },
  ...
  const [combinations] = useState<LoadCombinationResult[]>([
    ...
    {
      id: "LC2",
      name: "1.2DL + 1.2LL + 1.2EQx",
      type: "seismic",
      maxDisplacement: 18.2,
      maxStress: 198,
      maxUtilization: 0.79,
      critical: false,
    },
  ...
  return (
    <div className={`bg-white dark:bg-zinc-950 p-6 space-y-6 ${className}`}>
      {/* Header */}
      <motion.div
        initial={{ opacity: 0, y: -20 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <SummaryCard
          ...
          sparklineData={[0.65, 0.72, 0.68, 0.78, 0.85, 0.9, 0.88, 0.92, 0.98]}
        />
              ...
              data={[
                {
                  label: "Axial",
                  value: Math.max(
                    ...members.map((m) => Math.abs(m.axialForce)),
                  ),
                  color: CHART_COLORS.primary,
                },
                {
                  label: "Shear",
                  value: Math.max(...members.map((m) => Math.abs(m.shearY))),
                  color: CHART_COLORS.success,
                },
                {
                  label: "Moment",
                  value: Math.max(...members.map((m) => Math.abs(m.momentY))),
                  color: CHART_COLORS.warning,
                },
                {
                  label: "Stress",
                  value: Math.max(...members.map((m) => m.maxStress)),
                  color: CHART_COLORS.danger,
                },
              ]}
              horizontal
      ...
      <div className="bg-white/50 dark:bg-zinc-900/50 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            <p className="text-xs text-zinc-500 dark:text-zinc-400 mb-2">Shear Force Diagram</p>
            <ForceDiagram
              data={[
                { position: 0, value: 60 },
                { position: 1, value: 40 },
          ...
          <div>
            <p className="text-xs text-zinc-500 dark:text-zinc-400 mb-2">Bending Moment Diagram</p>
            <ForceDiagram
              data={[
                { position: 0, value: 0 },
                { position: 1, value: 80 },
          ...
          <div>
            <p className="text-xs text-zinc-500 dark:text-zinc-400 mb-2">Axial Force Diagram</p>
            <ForceDiagram
              data={[
                { position: 0, value: 15 },
                { position: 1, value: 15 },
