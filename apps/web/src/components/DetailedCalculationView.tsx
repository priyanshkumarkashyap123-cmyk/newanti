...
const DiagramRenderer: React.FC<DiagramRendererProps> = ({ diagram }) => {
  ...
  return (
    <div className="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
      <h4 className="text-sm font-semibold text-gray-700 mb-3">{diagram.title}</h4>
...
const CrossSectionDiagram: React.FC<{ data: any }> = ({ data }) => {
  ...
  return (
    <svg width={b * scale + 40} height={D * scale + 40} className="bg-white">
      {/* Concrete section */}
      <rect 
        x={20} y={20} 
...
const MomentDiagram: React.FC<{ data: any }> = ({ data }) => {
  return (
    <svg width={300} height={150} className="bg-white">
      {/* Beam line */}
...
const ReinforcementLayout: React.FC<{ data: any }> = ({ data }) => {
  return (
    <svg width={350} height={200} className="bg-white">
      {/* Beam elevation */}
...
const LoadingDiagram: React.FC<{ data: any }> = ({ data }) => {
  return (
    <svg width={300} height={180} className="bg-white">
      {/* Building outline */}
...
const CalculationStepCard: React.FC<CalculationStepCardProps> = ({ 
  ...
  return (
    <div className={`border rounded-lg mb-3 overflow-hidden ${
      step.status === 'FAIL' ? 'border-red-300' : 
      step.status === 'WARNING' ? 'border-yellow-300' : 'border-gray-200'
    }`}>
      {/* Header */}
      <div 
        className={`px-4 py-3 flex items-center justify-between cursor-pointer hover:bg-gray-50 ${
          step.status === 'FAIL' ? 'bg-red-50' : 
          step.status === 'WARNING' ? 'bg-yellow-50' : 'bg-white'
        }`}
        onClick={onToggle}
      >
      ...
      {isExpanded && (
        <div className="px-4 py-4 bg-gray-50 border-t border-gray-200">
          ...
          {step.reference?.code && (
            <div className="mb-3 flex items-center gap-2 text-sm">
              ...
              {step.reference?.clause && (
                <span className="text-gray-600">Clause {step.reference.clause}</span>
              )}
              {step.reference?.table && (
                <span className="text-gray-600">| {step.reference.table}</span>
              )}
              {step.reference?.figure && (
                <span className="text-gray-600">| {step.reference.figure}</span>
              )}
            </div>
          )}
          ...
          {step.values && Object.keys(step.values).length > 0 && (
            <div className="mb-4">
              <p className="text-xs text-gray-500 mb-2">Input Values:</p>
                  ...
                  return (
                    <div key={key} className="p-2 bg-white rounded border border-gray-200">
                      ...
                      <p className="font-mono font-medium">
                        ...
                        {value.unit && <span className="text-gray-500 text-sm ml-1">{value.unit}</span>}
                      </p>
                      {value.description && (
          ...
          <div className="p-3 bg-white rounded border-2 border-blue-200">
            ...
            <p className="text-xl font-bold text-blue-700">
              {typeof step.result.value === 'number' 
                ...
                : step.result.value}
              {step.result.unit && (
                <span className="text-sm font-normal text-gray-500 ml-2">{step.result.unit}</span>
              )}
            </p>
            {step.result.description && (
              <p className="text-sm text-gray-600 mt-1">{step.result.description}</p>
            )}
          </div>
        ...
        {activeTab === 'summary' && summary && (
          <div className="max-w-2xl mx-auto">
            ...
            <div className={`p-6 rounded-lg text-center mb-6 ${
              summary.status === 'ADEQUATE' ? 'bg-green-50 border border-green-200' :
              summary.status === 'INADEQUATE' ? 'bg-red-50 border border-red-200' :
            ...
            {/* Utilization Ratio */}
            <div className="mb-6">
              <h3 className="text-sm font-medium text-gray-700 mb-2">Utilization Ratio</h3>
