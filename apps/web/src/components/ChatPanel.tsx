        ...
        <AnimatePresence>
            {isOpen && (
                <motion.div
                    ...
                    className="fixed right-0 top-0 bottom-0 w-96 bg-slate-900 shadow-2xl z-50 flex flex-col border-l border-slate-700"
                >
                    {/* Header */}
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-900/50">
                        {messages.length === 0 && (
                            <div className="text-center py-12">
                                ...
                                <h3 className="text-slate-100 font-semibold mb-2">AI Engineering Assistant</h3>
                            ...
                            >
                                <div className={`max-w-[85%] ${message.role === 'user' ? 'order-2' : 'order-1'}`}>
                                    ...
                                    <div className={`flex items-start gap-2 ${message.role === 'user' ? 'flex-row-reverse' : ''}`}>
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${message.role === 'user' ? 'bg-blue-600' : 'bg-gray-800'
                                            }`}>
                                            {message.role === 'user'
                                                        ...
                                                        >
                                                            ...
                                                            >
                                                                <div className="flex items-center gap-2">
                                                                    ...
                                                                    <span className="font-medium text-sm text-slate-200">{suggestion.title}</span>
                                                                </div>
                                                                <ChevronDown className={`w-4 h-4 text-slate-500 transition-transform ${expandedSuggestion === suggestion.id ? 'rotate-180' : ''
                                                                    }`} />
                                                            </button>
                                                                    ...
                                                                    >
                                                                        <div className="p-3 space-y-3 text-sm">
                                                                            ...
                                                                            <div className="bg-blue-900/30 rounded p-2">
                                                                                <div className="text-xs font-medium text-blue-300 mb-1">Implementation:</div>
                                                                                <p className="text-blue-200">{suggestion.implementation}</p>
                                                                            </div>
                                    ...
                                    {/* Timestamp */}
                                    <div className={`text-xs text-slate-500 mt-1 ${message.role === 'user' ? 'text-right' : 'text-left ml-10'}`}>
                    ...
                    <div className="p-4 bg-slate-900 border-t border-slate-700">
                        <div className="flex items-center gap-2">
                            <input
                                ...
                                className="flex-1 px-4 py-3 bg-slate-800 text-slate-100 placeholder:text-slate-500 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                            />
                            <button
                                onClick={sendMessage}
                                disabled={!inputValue.trim() || isLoading}
                                className="w-12 h-12 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-700 rounded-xl flex items-center justify-center transition-colors"
                            >
...
function getReplacementSection(current: string): string {
    const upgrades: Record<string, string> = {
        ...
        'ISMB 200': 'Consider ISMB 250 or ISWB 200',
        'ISMB 250': 'Consider ISMB 300 or ISWB 250',
        'ISMB 300': 'Consider ISMB 350 or ISWB 300',
    };
...
export const AskAIButton: FC<AskAIButtonProps> = ({ onClick }) => {
    return (
        <button
            onClick={onClick}
            className="flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-xs font-medium rounded-lg hover:shadow-lg transition-all"
        >
...
export const ChatToggle: FC<ChatToggleProps> = ({ onClick, hasNotification }) => {
    return (
        <button
            onClick={onClick}
            className="fixed bottom-6 right-6 w-14 h-14 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-full shadow-lg flex items-center justify-center hover:shadow-xl transition-all z-40"
        >
