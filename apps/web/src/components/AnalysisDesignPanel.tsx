...
export const AnalysisDesignPanel: FC<AnalysisDesignPanelProps> = ({
    ...
    return (
        <div className="fixed inset-y-0 right-0 w-[500px] bg-surface-dark border-l border-border-dark shadow-2xl z-40 flex flex-col">
            ...
            {/* Summary Cards */}
            {analysisResults && (
                <div className="grid grid-cols-4 gap-2 p-4 bg-zinc-900/50">
                    <SummaryCard
                        label="Passed"
            ...
            <Tabs.Root value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col overflow-hidden">
                <Tabs.List className="flex border-b border-border-dark bg-zinc-900/30">
                    <Tabs.Trigger
                        value="forces"
                ...
                <Tabs.Content value="sections" className="flex-1 overflow-auto p-4">
                    <div className="space-y-4">
                        {/* Quick Apply */}
                        <div className="bg-zinc-900 rounded-lg p-4 border border-border-dark">
                            <h3 className="text-sm font-bold text-white mb-3">Assign Section & Material</h3>
                            ...
                            <div className="space-y-3">
                                <div>
                                    ...
                                    <select
                                        ...
                                        className="w-full bg-zinc-800 border border-border-dark rounded-lg px-3 py-2 text-white text-sm"
                                    >
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button
                                        ...
                                        className="px-4 py-2 text-sm font-medium rounded-lg bg-zinc-700 text-white hover:bg-zinc-600 transition-colors"
                                    >
                                        Apply to All
                                    </button>
                                </div>
                        ...
                        {defaultSection && (
                            <div className="bg-zinc-900 rounded-lg p-4 border border-border-dark">
                                <h3 className="text-sm font-bold text-white mb-3">Section Properties</h3>
                                {(() => {
                                    const section = getSectionById(defaultSection);
                            ...
                            <div className="space-y-2 max-h-48 overflow-y-auto">
                                {Array.from(members.keys()).map(memberId => {
                                    ...
                                    return (
                                        <div
                                            key={memberId}
                                            className="flex items-center justify-between py-1.5 px-2 rounded bg-zinc-800/50 text-xs"
                                        >
                                            <span className="text-white font-medium">M{memberId}</span>
...
const ForceValue: FC<{ label: string; value: number; unit: string }> = ({ label, value, unit }) => (
    <div className="bg-zinc-800 rounded px-2 py-1">
        ...
        <p className={`font-mono font-medium ${value < 0 ? 'text-blue-400' : 'text-white'}`}>
...
const DesignCheckRow: FC<{ check: { checkType: string; ratio: number; status: string; details: string } }> = ({ check }) => {
    ...
    return (
        <div className="flex items-center justify-between text-xs py-1 px-2 bg-zinc-800/50 rounded">
