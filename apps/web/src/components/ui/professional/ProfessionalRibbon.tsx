      ...
      `}>
        ...
        <button
          ...
          className={`
            ...
            ${isActive 
              ...
              : `hover:bg-zinc-700/50 hover:border-zinc-600/50 ${variantStyles[variant]}`}
        ...
        {hasDropdown && tool.splitButton && (
          <button
            onClick={() => setDropdownOpen(!dropdownOpen)}
            className={`
              flex items-center justify-center w-5 border-l border-zinc-700
              ${isActive ? 'bg-blue-600/20' : 'hover:bg-zinc-700/50'}
              transition-colors
            `}
          >
      ...
      <AnimatePresence>
        {dropdownOpen && hasDropdown && (
          <motion.div
            ...
            className="absolute top-full left-0 mt-1 min-w-[180px] bg-zinc-800 border border-zinc-700 rounded-lg shadow-xl z-50 py-1 overflow-hidden"
          >
            {tool.dropdown?.map((item, idx) => (
              <div key={item.id}>
                {item.divider && idx > 0 && <div className="h-px bg-zinc-700 my-1" />}
                <button
                  onClick={() => {
...
const ToolGroupComponent: FC<ToolGroupComponentProps> = ({ group, activeTool, onToolSelect }) => {
  return (
    <div className="flex flex-col h-full border-r border-zinc-700/50 px-2 pb-1 pt-1 last:border-r-0">
      {/* Tools Row */}
...
export const ProfessionalRibbon: FC<ProfessionalRibbonProps> = ({
  ...
  return (
    <div className="w-full bg-zinc-900 border-b border-zinc-800 flex flex-col select-none">
      {/* Title Bar */}
      <div className="h-9 flex items-center justify-between px-3 bg-zinc-950 border-b border-zinc-800">
        {/* Left: Logo + QAT */}
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-2 py-1 rounded bg-zinc-800 hover:bg-zinc-700 text-zinc-400 text-xs transition-colors"
          >
      ...
      {/* Tab Strip */}
      <div className="h-8 flex items-center gap-0.5 px-2 bg-zinc-900 border-b border-zinc-800">
        {RIBBON_TABS.map((tab) => {
          ...
          return (
            <button
              ...
              className={`
                ...
                ${isActive 
                  ? 'bg-zinc-800 text-white border-t-2 border-blue-500' 
        ...
        {!collapsed && currentTab && (
          ...
          >
            <div className="h-[72px] flex items-stretch px-2 py-1 bg-zinc-850">
              ...
              {isAnalyzing && (
                <div className="ml-auto flex items-center gap-3 px-4 border-l border-zinc-700">
                  <div className="flex items-center gap-2">
