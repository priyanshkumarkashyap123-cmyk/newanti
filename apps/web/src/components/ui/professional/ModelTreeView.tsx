...
const TreeSearchBar: FC<{
  ...
  <div className="relative px-2 py-1.5">
    ...
    <input
      ...
      className="w-full pl-8 pr-3 py-1.5 text-xs bg-zinc-800 border border-zinc-700 rounded-md text-zinc-300 placeholder-zinc-500 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
    />
    {value && (
      <button
        onClick={() => onChange('')}
...
const TreeToolbar: FC<{
  ...
  <div className="flex items-center gap-1 px-2 py-1 border-b border-zinc-800">
    ...
    {onRefresh && (
      <button
        onClick={onRefresh}
        className="p-1 text-zinc-400 hover:text-zinc-300 hover:bg-zinc-800 rounded ml-auto"
        title="Refresh"
      >
    ...
    >
      ...
      <button
        ...
        className={`w-4 h-4 flex items-center justify-center flex-shrink-0 ${
          hasChildren ? 'text-zinc-400 hover:text-zinc-300' : 'invisible'
        }`}
      >
        {hasChildren && (
      ...
      {node.badge !== undefined && (
        <span className="px-1.5 py-0.5 text-[10px] bg-zinc-700 text-zinc-300 rounded-full">
          {node.badge}
        </span>
      )}
      ...
      {onToggleVisibility && (
        <button
          ...
          className={`p-0.5 rounded hover:bg-zinc-700 ${
            node.visible === false ? 'text-zinc-500' : 'text-zinc-400'
          }`}
        >
      ...
      {onToggleLock && (
        <button
          ...
          className={`p-0.5 rounded hover:bg-zinc-700 ${
            node.locked ? 'text-amber-400' : 'text-zinc-400'
          }`}
        >
...
export const ModelTreeView: FC<ModelTreeViewProps> = ({
  ...
  return (
    <div className="h-full flex flex-col bg-zinc-900 border-r border-zinc-800">
      ...
      {/* Selection Info */}
      {selectedIds.size > 0 && (
        <div className="px-3 py-1.5 border-t border-zinc-800 text-xs text-zinc-400">
