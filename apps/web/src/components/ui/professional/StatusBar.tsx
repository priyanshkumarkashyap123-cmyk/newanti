...
const CoordinateDisplay: FC<{
  ...
  >
    <Crosshair className="w-3.5 h-3.5 text-slate-500 dark:text-slate-400" />
    <div className="flex items-center gap-1.5 text-xs font-mono">
      ...
      <span className="text-slate-500 dark:text-slate-400 text-[10px]">{unit}</span>
    </div>
  </button>
));
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 4 }}
            className="absolute bottom-full left-0 mb-1 w-48 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl overflow-hidden"
          >
            <div className="px-3 py-2 border-b border-slate-200 dark:border-slate-800 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
              Unit System
            </div>
            {UNIT_SYSTEMS.map(system => (
              ...
              >
                ...
                <span className="text-slate-500 dark:text-slate-400">{system.length}, {system.force}</span>
              </button>
            ))}
          </motion.div>
        )}
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 4 }}
            className="absolute bottom-full right-0 mb-1 w-80 max-h-64 overflow-auto bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl"
          >
            <div className="px-3 py-2 border-b border-slate-200 dark:border-slate-800 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
              Notifications
            </div>
            {notifications.map(notification => {
              ...
              return (
                <div
                  key={notification.id}
                  className="flex items-start gap-2 px-3 py-2 border-b border-slate-200 dark:border-slate-800 last:border-0 hover:bg-slate-200/50 dark:hover:bg-slate-800/50"
                >
      ...
      <AnimatePresence>
        {showAnalysisPanel && analysisProgress?.status === 'running' && (
          <motion.div
            ...
            className="bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 overflow-hidden"
          >
