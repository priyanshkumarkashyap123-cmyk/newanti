  ...
  return (
    <div
      ...
      className={`
        ...
        bg-zinc-700/30 hover:bg-blue-500/50
    ...
    >
      ...
      <div className="flex items-center gap-0.5">
        {config.collapsible && (
          <button
            onClick={onCollapse}
            className="p-1 rounded hover:bg-zinc-600/50 text-zinc-400 hover:text-white transition-colors"
        ...
        <button
          ...
          className={`
            ...
            ${state.pinned ? 'text-blue-400' : 'text-zinc-400 hover:text-white'}
          `}
          ...
          <AnimatePresence>
            {menuOpen && (
              <motion.div
                ...
                className="absolute right-0 top-full mt-1 w-40 bg-zinc-800 border border-zinc-700 rounded-lg shadow-xl z-50 py-1"
              >
                {config.floatable && (
                  <button
        ...
        {config.closable && (
          <button
            onClick={onClose}
            className="p-1 rounded hover:bg-red-600/30 text-zinc-400 hover:text-red-400 transition-colors"
            title="Close"
          >
            <X className="w-3 h-3" />
          </button>
...
export const DockContainer: FC<DockContainerProps> = ({ position, children }) => {
  ...
  return (
    <div
      className={`
        flex bg-zinc-900 border-zinc-800
        ${isHorizontal 
    ...
    >
      ...
      <div
        className={`
          ...
          ${isHorizontal ? 'h-8 w-full border-b border-zinc-800' : 'w-8 h-full border-r border-zinc-800'}
          bg-zinc-850 cursor-pointer hover:bg-zinc-800
        `}
...
export const PanelTabBar: FC<PanelTabBarProps> = ({
  ...
  return (
    <div className="flex items-center h-8 bg-zinc-800 border-b border-zinc-700 overflow-x-auto">
      {panels.map((id) => {
        const config = configs.get(id);
...
export const FloatingPanel: FC<FloatingPanelProps> = ({ id, children }) => {
  ...
  return (
    <motion.div
      ...
      onClick={() => setActivePanel(id)}
      className={`
        fixed z-50 bg-zinc-900 border rounded-lg shadow-2xl overflow-hidden
        ${isActive ? 'border-blue-500/50' : 'border-zinc-700'}
      `}
