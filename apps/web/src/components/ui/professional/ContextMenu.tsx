...
const MenuItemComponent: FC<MenuItemComponentProps> = memo(({
  ...
  if (item.type === 'divider') {
    return <div className="h-px bg-zinc-700 my-1 mx-2" />;
  }
  ...
  return (
    <div
      ...
      className={`
        ...
        ${openSubmenuId === item.id ? 'bg-zinc-700/50' : ''}
      `}
      onClick={handleClick}
    ...
    >
      ...
      {Icon && !isCheckbox && !isRadio && (
        <Icon className={`w-4 h-4 flex-shrink-0 ${item.danger ? 'text-red-400' : 'text-zinc-400'}`} />
      )}
...
const Submenu: FC<SubmenuProps> = ({
  ...
  return (
    <motion.div
      ...
      className="fixed z-[10001] min-w-[180px] py-1 bg-zinc-900 border border-zinc-700 rounded-lg shadow-xl"
      style={{
        left: position.x,
        top: position.y
      }}
...
export const ContextMenu: FC<ContextMenuProps> = ({
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.1 }}
      className="fixed z-[10000] py-1 bg-zinc-900 border border-zinc-700 rounded-lg shadow-xl overflow-hidden"
      style={{
