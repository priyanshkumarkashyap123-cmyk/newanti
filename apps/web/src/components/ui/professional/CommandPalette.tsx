...
const CATEGORY_CONFIG: Record<CommandCategory, { icon: React.ElementType; color: string; label: string }> = {
  ...
  settings: { icon: Settings, color: 'text-slate-500 dark:text-slate-400', label: 'Settings' },
  help: { icon: Command, color: 'text-indigo-400', label: 'Help' },
  recent: { icon: Clock, color: 'text-slate-500 dark:text-slate-400', label: 'Recent' },
};
  ...
  return (
    <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-200 dark:border-slate-700">
      ...
      <div className="flex items-center gap-1 text-[10px] text-slate-500 dark:text-slate-400">
        ...
        <span>navigate</span>
        <kbd className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 rounded ml-2">Enter</kbd>
        <span>select</span>
      </div>
    </div>
  );
  ...
  return (
    <div
      className={`
        ...
        ${isSelected ? 'bg-blue-500/20' : 'hover:bg-slate-100 dark:hover:bg-slate-800/50'}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="text-sm text-slate-800 dark:text-slate-200 truncate">{command.label}</div>
        {command.description && (
          <div className="text-xs text-slate-500 dark:text-slate-400 truncate">{command.description}</div>
        )}
      </div>
      <div className="flex items-center gap-2">
        <span className={`text-[10px] px-1.5 py-0.5 rounded ${categoryConfig.color} bg-slate-100 dark:bg-slate-800`}>
          {categoryConfig.label}
        </span>
        {command.shortcut && (
          <kbd className="px-1.5 py-0.5 text-[10px] bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded">
            {command.shortcut}
          </kbd>
        )}
      </div>
...
const FileItem: FC<{
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="text-sm text-slate-800 dark:text-slate-200 truncate">{file.name}</div>
...
const SymbolItem: FC<{
  ...
  return (
    <div
      ...
      onMouseEnter={onMouseEnter}
    >
      <Icon className={`w-4 h-4 ${typeColors[symbol.type] || 'text-slate-500 dark:text-slate-400'}`} />
      <div className="flex-1 min-w-0">
        <div className="text-sm text-slate-800 dark:text-slate-200 truncate">{symbol.name}</div>
        {symbol.location && (
          <div className="text-xs text-slate-500 dark:text-slate-400 truncate">{symbol.location}</div>
        )}
      </div>
      <span className={`text-[10px] px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 ${typeColors[symbol.type] || 'text-slate-500 dark:text-slate-400'}`}>
        {symbol.type}
      </span>
    </div>
  );
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-[600px] max-h-[60vh] p-0 gap-0 top-[15vh] translate-y-0 overflow-hidden">
          ...
          {/* Mode Tabs */}
          <div className="flex items-center gap-2 px-4 py-2 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50">
            {[
              ...
              <button
                ...
                `}
              >
                {tab.prefix && <span className="text-slate-500 dark:text-slate-500 mr-1">{tab.prefix}</span>}
                {tab.label}
              </button>
            ))}
          </div>
            ...
            {mode === 'goto' ? (
              <div className="px-4 py-8 text-center text-slate-500 dark:text-slate-400">
                ...
                {query && !isNaN(parseInt(query)) && (
                  <p className="mt-2 text-slate-500 dark:text-slate-400">
                    Press <kbd className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 rounded text-xs">Enter</kbd> to go to line {query}
                  </p>
                )}
              </div>
              ...
              query ? (
                ...
                Object.entries(groupedCommands || {}).map(([category, cmds]) => {
                  ...
                  return (
                    <div key={category}>
                      <div className="flex items-center gap-2 px-4 py-1.5 bg-slate-100 dark:bg-slate-800/50 sticky top-0">
          ...
          {/* Footer */}
          <div className="px-4 py-2 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 flex items-center justify-between text-[10px] text-slate-500 dark:text-slate-400">
            <div className="flex items-center gap-4">
              <span>
                <kbd className="px-1 py-0.5 bg-slate-100 dark:bg-slate-800 rounded">Ctrl</kbd>+
                <kbd className="px-1 py-0.5 bg-slate-100 dark:bg-slate-800 rounded">K</kbd> Quick search
              </span>
              <span>
                <kbd className="px-1 py-0.5 bg-slate-100 dark:bg-slate-800 rounded">Ctrl</kbd>+
                <kbd className="px-1 py-0.5 bg-slate-100 dark:bg-slate-800 rounded">P</kbd> Command palette
              </span>
            </div>
