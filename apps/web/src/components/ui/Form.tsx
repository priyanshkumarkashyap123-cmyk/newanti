...
export function FormField({ label, error, hint, required, className = '', children }: FormFieldProps) {
  ...
  return (
    <div className={`space-y-1.5 ${className}`}>
      ...
      {hint && !error && (
        <p id={`${id}-hint`} className="flex items-center gap-1.5 text-sm text-slate-400">
    ...
    return (
      <div className="space-y-1.5">
        ...
        <div className="flex justify-between">
          ...
          {showCount && maxLength && (
            <span className={`text-sm ${charCount >= maxLength ? 'text-red-400' : 'text-slate-400'}`}>
              {charCount}/{maxLength}
            </span>
          )}
        </div>
    ...
    return (
      <div className={`flex items-start gap-3 ${className}`}>
        <input
          ...
          className={`
            mt-1 w-4 h-4 
            bg-slate-800 border-slate-600 rounded
            text-blue-500 
        ...
        <div className="flex-1">
          ...
          {description && (
            <p id={`${id}-description`} className="text-sm text-slate-400 mt-0.5">
              {description}
            </p>
          )}
          {error && (
