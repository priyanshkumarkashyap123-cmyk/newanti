...
export const StatCard: FC<StatCardProps> = ({
    ...
    return (
        <motion.div
            ...
            whileHover={{ y: -2 }}
            className={`
                p-5 bg-[#0b1326] border border-[#1a2333] rounded-xl
        ...
        >
            ...
            <div className="flex items-end gap-2 mb-1">
                <span className="text-3xl font-bold text-[#dae2fd]">{value}</span>
                {subtitle && (
                    <span className="text-sm text-[#869ab8] mb-1">{subtitle}</span>
                )}
            </div>

            {trend && (
                <div className={`flex items-center gap-1 text-sm ${trend.value > 0 ? 'text-green-400' : trend.value < 0 ? 'text-red-400' : 'text-[#869ab8]'
                    }`}>
                    {trend.value > 0 ? (
                    ...
                    {trend.label && (
                        <span className="text-[#869ab8]">{trend.label}</span>
                    )}
                </div>
            )}
        </motion.div>
        ...
        <div className={`${className}`}>
                        ...
                        <div key={item.label} className="flex-1 flex flex-col items-center gap-1">
                            {showValues && (
                                <motion.span
                                    ...
                                    className="text-xs text-[#869ab8] font-mono"
                                >
                                    {item.value}
                                </motion.span>
                            )}
            ...
            {showLabels && (
                <div className="flex gap-2 mt-2">
                    {data.map((item) => (
                        <span key={item.label} className="flex-1 text-xs text-[#869ab8] text-center truncate">
                            {item.label}
                        </span>
                    ))}
                </div>
        ...
        <div className={`relative ${className}`} style={{ width: size, height: size / 2 + 20 }}>
            ...
            <div className="absolute bottom-0 left-1/2 -translate-x-1/2 text-center">
                <div className="text-2xl font-bold text-[#dae2fd]">
                    {value.toFixed(0)}
                    {unit && <span className="text-sm text-[#869ab8] ml-1">{unit}</span>}
                </div>
                {label && <div className="text-xs text-[#869ab8]">{label}</div>}
            </div>
        </div>
    );
};
