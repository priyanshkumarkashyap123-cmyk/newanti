...
function ContextMenu({ items, position, onClose }: ContextMenuProps) {
    ...
    const renderMenuItem = (item: ContextMenuItem, index: number) => {
        if (item.type === 'divider') {
            return <div key={index} className="my-1 h-px bg-zinc-700" />;
        }
        ...
        return (
            <div key={item.id || index} className="relative">
                <button
                    ...
                    `}
                >
                    <span className="w-4 h-4 flex-shrink-0 text-zinc-400">
                        {item.icon}
                    </span>
                ...
                {item.submenu && isHovered && (
                    <div
                        className="absolute left-full top-0 ml-1 bg-zinc-900/95 backdrop-blur-xl border border-zinc-700 rounded-lg shadow-xl py-1 min-w-[160px]"
    ...
    return createPortal(
        <div
            ref={menuRef}
            className="fixed z-[9999] bg-zinc-900/95 backdrop-blur-xl border border-zinc-700 rounded-lg shadow-2xl py-1 min-w-[200px] animate-in fade-in zoom-in-95 duration-100"
            style={{
