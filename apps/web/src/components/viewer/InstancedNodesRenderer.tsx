...
export const InstancedNodesRenderer: React.FC = () => {
  ...
  const geometry = useMemo(() => {
    return GpuResourcePool.getSphereGeometry(NODE_RADIUS, SPHERE_SEGMENTS);
  }, []);
  ...
  useEffect(() => {
    ...
    if (!mesh.geometry.attributes.instanceColor) {
      ...
      (
        mesh.geometry.attributes.instanceColor as THREE.InstancedBufferAttribute
      ).array = colorArray;
  ...
  useEffect(() => {
    ...
    const mesh = meshRef.current;
    const colorAttribute = mesh.geometry.attributes
