...
function hasSupport(node: { restraints?: { fx?: boolean; fy?: boolean; fz?: boolean; mx?: boolean; my?: boolean; mz?: boolean } }): boolean {
...
export const UltraLightNodesRenderer: React.FC = React.memo(() => {
    ...
    const visibleNodes = useMemo(() => {
        ...
        if (config.showOnlySupportNodes) {
            ...
            console.log(`[UltraLightNodes] Showing ${supports.length} support nodes out of ${nodeArray.length} total`);
            return supports;
        }
    ...
    const material = useMemo(() => {
        if (nodeCount > THRESHOLDS.EXTREME) {
            return new THREE.MeshBasicMaterial({
                color: COLORS.nodeSupport, // Green for supports
            });
        }
    ...
    useEffect(() => {
        ...
        const processChunk = () => {
            ...
            if (currentIndex < toRender) {
                ...
                console.log(`[UltraLightNodes] Rendered ${toRender} nodes with ${config.sphereSegments}-segment spheres`);
            }
        };
    ...
    useEffect(() => {
        ...
        colorAttr.setXYZ(hoveredInstanceId, COLORS.nodeHover.r, COLORS.nodeHover.g, COLORS.nodeHover.b);
