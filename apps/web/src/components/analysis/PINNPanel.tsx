...
export const PINNPanel: FC = () => {
    ...
    return (
        <div className="h-full flex flex-col bg-[#0b1326] p-4 space-y-6 text-slate-700 dark:text-slate-200">
            {/* Header */}
            <div className="flex items-center gap-3 border-b border-[#1a2333] pb-4">
                <div className="p-2 bg-gradient-to-br from-purple-600 to-indigo-600 rounded-lg">
                    <Brain className="w-5 h-5 text-white" />
                </div>
                <div>
                    <h2 className="text-sm font-bold text-[#dae2fd]">AI Physics Engine</h2>
                    <p className="text-xs text-[#869ab8]">Train Neural Networks to solve beams</p>
                </div>
            </div>

            {/* Config */}
            <div className="space-y-4">
                <h3 className="text-xs font-bold text-[#869ab8] uppercase tracking-wider">Configuration</h3>

                <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                        <label className="text-[10px] text-[#869ab8]">Length (m)</label>
                        <input
                            type="number"
                            className="w-full bg-[#0b1326] border border-[#1a2333] rounded px-2 py-1 text-xs text-[#dae2fd]"
                            value={beamLength}
                            onChange={(e) => setBeamLength(Number(e.target.value))}
                        />
                    </div>
                    <div className="space-y-1">
                        <label className="text-[10px] text-[#869ab8]">Load (N/m)</label>
                        <input
                            type="number"
                            className="w-full bg-[#0b1326] border border-[#1a2333] rounded px-2 py-1 text-xs text-[#dae2fd]"
                            value={loadMag}
                ...
                <div className="space-y-1">
                    <label className="text-[10px] text-[#869ab8]">Training Epochs (Complexity)</label>
                    <input
                        type="range"
                        min="500" max="5000" step="500"
                        className="w-full h-1 bg-[#131b2e] rounded-lg appearance-none cursor-pointer"
                        value={epochs}
                ...
                <div className="space-y-3 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="p-3 bg-green-500/10 border border-green-500/20 rounded flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <div className="text-[10px] text-[#869ab8]">ID: {modelId}</div>
                        </div>
                    </div>

                    {prediction && (
                        <div className="space-y-2">
                            <h3 className="text-xs font-bold text-[#869ab8] uppercase tracking-wider flex items-center gap-2">
                                <BarChart className="w-3 h-3" />
                                Instant Prediction
                            </h3>
                            <div className="grid grid-cols-2 gap-2 text-xs">
                                <div className="p-2 bg-[#0b1326] rounded border border-[#1a2333] text-center">
                                    <div className="text-[#869ab8] text-[10px]">Max Deflection</div>
                                    <div className="font-mono text-purple-400 text-sm">
                                        {(prediction.max_deflection * 1000).toFixed(2)} mm
                                    </div>
                                </div>
                                <div className="p-2 bg-[#0b1326] rounded border border-[#1a2333] text-center">
                                    <div className="text-[#869ab8] text-[10px]">Inference Time</div>
