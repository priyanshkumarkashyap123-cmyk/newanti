...
export const LearningAssistant: FC = () => {
    ...
    return (
        <div className="fixed bottom-4 right-4 w-80 bg-white/95 dark:bg-zinc-900/95 backdrop-blur-sm border border-zinc-200/50 dark:border-zinc-700/50 rounded-xl shadow-2xl flex flex-col z-30 max-h-[70vh]">
            ...
            <div className="flex items-center justify-between px-4 py-3 bg-indigo-600/20 border-b border-indigo-500/20 flex-shrink-0">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-bold text-zinc-900 dark:text-white">AI Learning Assistant</span>
                </div>
                <button
            ...
            <div className="p-4 space-y-4 flex-1 overflow-y-auto">
                ...
                {skills.length > 0 && (
                    <div>
                        <h4 className="text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-2 flex items-center gap-1">
                            <TrendingUp className="w-3 h-3" /> Recent Progress
                        </h4>
                        <div className="space-y-2">
                            {skills.map(skill => (
                                <div key={skill.skill} className="bg-white dark:bg-zinc-950 p-2 rounded border border-zinc-200 dark:border-zinc-800">
                                    <div className="flex justify-between text-xs mb-1">
                                        <span className="text-zinc-700 dark:text-zinc-200 capitalize truncate mr-2">{skill.skill.replace('_', ' ')}</span>
                ...
                <div>
                    ...
                    {recommendations.length === 0 ? (
                        <div className="text-xs text-zinc-500 dark:text-zinc-400 italic p-2 bg-zinc-100/50 dark:bg-zinc-800/50 rounded">
                            Interact with tools to get personalized suggestions...
                        </div>
                    ) : (
                        <div className="space-y-2">
                            {recommendations.map((rec, idx) => (
                                <div key={idx} className="bg-zinc-100/50 dark:bg-zinc-800/50 p-3 rounded border border-zinc-200/50 dark:border-zinc-700/50 hover:bg-zinc-200 dark:hover:bg-zinc-800 transition-colors">
                                    <div className="flex items-start gap-2">
                                        ...
                                        <div>
                                            <div className="text-xs font-bold text-zinc-800 dark:text-zinc-100">{rec.title}</div>
                                            <div className="text-[11px] text-zinc-600 dark:text-zinc-300 mt-0.5">{rec.description}</div>
                                            <div className="mt-2 flex gap-2">
                                                <button className="px-2 py-1 bg-indigo-600/20 text-indigo-400 text-[10px] rounded hover:bg-indigo-600/30">
                                                    Start Lesson
                                                </button>
                                                <button className="px-2 py-1 bg-zinc-200/50 dark:bg-zinc-700/50 text-zinc-500 dark:text-zinc-400 text-[10px] rounded hover:bg-zinc-200 dark:hover:bg-zinc-700">
                                                    Dismiss
                                                </button>
                                            </div>
                                        </div>
