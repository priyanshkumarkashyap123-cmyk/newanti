...
const Toast: FC<{ state: ToastState }> = ({ state }) => {
    ...
    return (
        <div className={`fixed bottom-4 right-4 ${colors[state.type]} text-white px-4 py-2 rounded-lg shadow-lg flex items-center gap-2 z-50`}>
                        ...
                        return (
                            <div key={category.id} className="border-b border-zinc-800/50">
                                ...
                                <button
                                    ...
                                    className="w-full flex items-center gap-2 px-3 py-2 text-sm font-medium text-zinc-300 hover:bg-zinc-800 transition-colors"
                                >
                                    {isExpanded
                                        ? <ChevronDown className="w-4 h-4 text-zinc-400" />
                                        : <ChevronRight className="w-4 h-4 text-zinc-400" />
                                    }
                                    <span className="text-zinc-400">{category.icon}</span>
                                    {category.name}
