...
export const EngineeringWorkspace: FC<EngineeringWorkspaceProps> = ({
    ...
    return (
        <div className="h-screen flex flex-col overflow-hidden bg-white dark:bg-slate-900 text-slate-900 dark:text-white font-display">
            {/* Top Menu Bar */}
            <header className="h-[30px] bg-slate-100 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex items-center px-4 justify-between shrink-0 select-none z-50">
                ...
                <div className="flex items-center gap-3">
                    <span className="text-[10px] text-slate-500 dark:text-slate-400">v4.2.0-pro</span>
                </div>
            </header>
                ...
                <aside className="w-[50px] bg-slate-100 dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 flex flex-col items-center py-2 gap-2 shrink-0 z-40">
                    {tools.map((tool) => (
                        ...
                        >
                            ...
                            <div className="absolute left-10 bg-white dark:bg-slate-900 text-xs px-2 py-1 rounded border border-slate-200 dark:border-slate-700 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-50">
                                {tool.tooltip}
                            </div>
                        </button>
                    ))}
                ...
                <div className="flex-1 flex flex-col">
                    <div className="flex-1 flex overflow-hidden">
                        ...
                        <div className="flex-1 flex flex-col min-w-0">
                            {/* 3D Viewport */}
                            <div className={`${resultsPanel ? 'flex-[2]' : 'flex-1'} relative bg-white dark:bg-slate-900 overflow-hidden`}>
                                {children}
                            </div>

                            {/* Results Panel (Bottom) */}
                            {resultsPanel && (
                                <div className="flex-1 bg-slate-100 dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 flex flex-col min-h-[200px]">
                                    {resultsPanel}
                                </div>
                            )}
                        </div>
                        ...
                        {propertiesPanel && (
                            <div className="w-[300px] bg-slate-100 dark:bg-slate-800 border-l border-slate-200 dark:border-slate-700 overflow-y-auto shrink-0">
                                {propertiesPanel}
                            </div>
                        )}
                    </div>
            ...
            {/* Status Bar */}
            <footer className="h-8 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between px-4 shrink-0 z-20">
                <div className="flex items-center gap-4">
                    ...
                    <span className="text-xs text-slate-500 dark:text-slate-400 font-mono">X: 0.00 Y: 0.00 Z: 0.00</span>
                </div>
                <div className="flex items-center gap-4">
                    <span className="text-xs text-slate-500 dark:text-slate-400">0 Nodes, 0 Members</span>
                    <div className="h-3 w-px bg-slate-200 dark:bg-slate-700"></div>
                    <span className="text-xs text-slate-500 dark:text-slate-400">Memory: 45MB</span>
                </div>
            </footer>
            ...
            {showTutorial && (
                <div className="fixed inset-0 z-50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm flex items-center justify-center">
                    <div className="bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl shadow-black max-w-lg w-full overflow-hidden relative">
                        ...
                        <div className="p-8 flex flex-col items-center text-center">
                            ...
                            <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-3">Welcome to BeamLab!</h2>
