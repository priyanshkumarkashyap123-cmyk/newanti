...
const FormatCard: React.FC<FormatCardProps> = ({ format, selected, onSelect }) => (
    ...
    >
        <div className="flex items-center gap-3">
            <div className={`p-2 rounded-lg ${selected ? 'bg-cyan-500/20 text-cyan-400' : 'bg-slate-700 text-slate-400'}`}>
                {format.icon}
            </div>
            <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-400 truncate">{format.description}</p>
            </div>
            {selected && (
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
                {/* Precision */}
                <div>
                    <label className="block text-xs text-slate-400 mb-1">Decimal Precision</label>
                    <select
                        value={options.precision}
                        onChange={(e) => updateOption('precision', parseInt(e.target.value))}
                        className="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-white text-sm focus:border-cyan-500"
                    >
                ...
                {(category === 'model' || category === 'results') && (
                    <div>
                        <label className="block text-xs text-slate-400 mb-1">Coordinate System</label>
                        <select
                ...
                {category === 'model' && (
                    <div>
                        <label className="block text-xs text-slate-400 mb-1">Scale Factor</label>
                        <select
                            value={options.scale}
            ...
            <div className="space-y-2">
                <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    />
                    <span className="text-sm text-slate-300">Include metadata (project info, timestamp)</span>
                </label>
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                    <input
                        ...
                        className="rounded bg-slate-700 border-slate-600 text-cyan-500"
                    />
                    <span className="text-sm text-slate-300">Include units in headers</span>
                </label>
                    ...
                    <>
                        <label className="flex items-center gap-2 cursor-pointer">
                            <input
                                ...
                                className="rounded bg-slate-700 border-slate-600 text-cyan-500"
                            />
                            <span className="text-sm text-slate-300">Include all load cases</span>
                        </label>
                        ...
                        <label className="flex items-center gap-2 cursor-pointer">
                            <input
                                ...
                                className="rounded bg-slate-700 border-slate-600 text-cyan-500"
                            />
                            <span className="text-sm text-slate-300">Include envelope values</span>
                        </label>
                        ...
                        <label className="flex items-center gap-2 cursor-pointer">
                            <input
                                ...
                                className="rounded bg-slate-700 border-slate-600 text-cyan-500"
                            />
                            <span className="text-sm text-slate-300">Split into separate files by member</span>
                        </label>
                    </>
                )}
                ...
                {(category === 'model' || category === 'results') && (
                    <label className="flex items-center gap-2 cursor-pointer">
                        <input
                            ...
                            className="rounded bg-slate-700 border-slate-600 text-cyan-500"
                        />
                        <span className="text-sm text-slate-300">Compress output (ZIP)</span>
                    </label>
                )}
            </div>
        </div>
                ...
                <div className="flex-1 min-w-0">
                    ...
                    {job.status === 'processing' && (
                        <div className="mt-2">
                            ...
                            <span className="text-xs text-slate-400 mt-1">{job.progress}% complete</span>
                        </div>
                    )}
...
export const ExportManager: React.FC<ExportManagerProps> = ({
    ...
    return (
        <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden">
            ...
            <div className="flex">
                {/* Category Sidebar */}
                <div className="w-56 border-r border-slate-800 p-4">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        {selectedFormat && (
                            <span className="text-sm text-slate-400">
                                Output: <span className="text-white">{projectName}{selectedFormat.extension}</span>
                            </span>
                        )}
                    </div>
                </div>
