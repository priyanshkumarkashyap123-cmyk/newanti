...
export function IntegratedEngineeringDashboard({
  ...
  const renderStatusBadge = (status: string) => {
    const configs = {
      ...
      pending: { icon: <Clock className="w-3.5 h-3.5" />, class: 'bg-slate-500/20 text-slate-500 dark:text-slate-400 border-slate-500/30' },
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white dark:from-slate-950 via-slate-100 dark:via-slate-900 to-white dark:to-slate-950 text-slate-900 dark:text-white p-6">
      {/* Header */}
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <main className="col-span-8 space-y-6">
          ...
          <div className="grid grid-cols-4 gap-4">
            <motion.div
              ...
              className="p-4 rounded-xl bg-white/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 backdrop-blur-sm"
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-slate-500 dark:text-slate-400 text-sm">Overall Progress</span>
                <Target className="w-4 h-4 text-blue-400" />
              </div>
              <div className="flex items-end gap-2">
                <span className="text-3xl font-bold text-slate-900 dark:text-white">{overallProgress}%</span>
              </div>
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-slate-500 dark:text-slate-400 text-sm">Design Checks</span>
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-slate-500 dark:text-slate-400 text-sm">Max Utilization</span>
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-slate-500 dark:text-slate-400 text-sm">Members</span>
                <Grid3X3 className="w-4 h-4 text-purple-400" />
              </div>
              <div className="flex items-end gap-2">
                <span className="text-3xl font-bold text-slate-900 dark:text-white">{SAMPLE_METRICS.designedMembers}</span>
                <span className="text-lg text-slate-500 dark:text-slate-400 mb-0.5">/{SAMPLE_METRICS.totalMembers}</span>
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Designed members</p>
            </motion.div>
          </div>
              ...
              <button type="button"
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                  selectedCategory === cat.id
                    ...
                    : 'bg-slate-100/50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 hover:bg-slate-200/50 dark:hover:bg-slate-700/50 hover:text-slate-900 dark:hover:text-white border border-slate-200/50 dark:border-slate-700/50'
                }`}
              >
                {cat.icon}
                {cat.label}
                ...
                <motion.div
                  ...
                  className={`group p-5 rounded-xl border backdrop-blur-sm cursor-pointer transition-all hover:shadow-lg ${
                    selectedModule === module.id
                      ...
                      : 'bg-white/50 dark:bg-slate-900/50 border-slate-200 dark:border-slate-800 hover:border-slate-400 dark:hover:border-slate-700 hover:bg-slate-200/50 dark:hover:bg-slate-800/50'
                  }`}
                >
                  ...
                  <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-1 group-hover:text-blue-400 transition-colors">
                    {module.name}
                  </h3>
                  ...
                  <div className="mb-3">
                    <div className="flex justify-between text-xs mb-1">
                      ...
                      <span className="text-slate-500 dark:text-slate-400">{module.progress}%</span>
                    </div>
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between">
                    <span className="text-xs text-slate-500 dark:text-slate-400">Click to open</span>
        ...
        <aside className="col-span-4 space-y-6">
          ...
          <motion.div
            ...
            className="p-5 rounded-xl bg-white/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 backdrop-blur-sm"
          >
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span className="text-slate-900 dark:text-white font-medium">{SAMPLE_METRICS.concreteVolume} m³</span>
                </div>
              ...
              <div>
                <div className="flex justify-between text-sm mb-1">
                  <span className="text-slate-500 dark:text-slate-400">Steel Reinforcement</span>
                  <span className="text-slate-900 dark:text-white font-medium">{(SAMPLE_METRICS.steelWeight / 1000).toFixed(2)} MT</span>
                </div>
                <div className="h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
          ...
          >
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto pr-2">
              {RECENT_CHECKS.map((check) => (
                <div
                  key={check.id}
                  className="flex items-center justify-between p-3 rounded-lg bg-slate-100/50 dark:bg-slate-800/50 hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors"
                >
                  <div className="flex items-center gap-3">
                    {renderCheckStatus(check.status)}
                    <div>
                      <p className="text-sm font-medium text-slate-900 dark:text-white">{check.component}</p>
                      <p className="text-xs text-slate-500 dark:text-slate-400">{check.check}</p>
                    </div>
                  </div>
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="text-sm font-medium text-slate-900 dark:text-white mb-1">Calculation Engine v3.0</h4>
