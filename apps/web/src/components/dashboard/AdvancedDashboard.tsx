...
const ProjectCard: React.FC<{
  ...
  return (
    ...
    >
      {/* Thumbnail / Preview */}
      <div className="relative h-32 bg-gradient-to-br from-gray-100 to-gray-50 p-4">
      ...
      <div className="flex border-t border-gray-100">
        <button
          onClick={onOpen}
          className="flex-1 py-2 text-sm font-medium text-gray-600 hover:bg-gray-50 hover:text-gray-900 transition-colors"
        >
          Open
        </button>
...
const AnalysisJobRow: React.FC<{ job: AnalysisJob }> = ({ job }) => {
  ...
  return (
    <motion.div
      ...
      className="flex items-center gap-4 rounded-lg border border-gray-100 bg-white p-3 hover:bg-gray-50"
    >
      <div className={`h-2 w-2 rounded-full ${job.status === 'running' ? 'animate-pulse bg-blue-500' : job.status === 'complete' ? 'bg-green-500' : 'bg-gray-300'}`} />
      <div className="flex-1 min-w-0">
        <p className="font-medium text-gray-900 truncate">{job.projectName}</p>
      ...
      {job.status === 'running' && (
        <div className="w-24">
          ...
          <p className="mt-0.5 text-xs text-gray-500 text-center">{job.progress}%</p>
        </div>
      )}
      {job.status === 'complete' && job.duration && (
        <span className="text-sm text-gray-500">{job.duration}s</span>
      )}
...
const NotificationItem: React.FC<{ notification: Notification; onRead: () => void }> = ({ notification, onRead }) => {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, x: 0 }}
      onClick={onRead}
      className={`cursor-pointer border-l-4 rounded-r-lg p-3 ${typeStyles[notification.type]} ${!notification.read ? 'ring-2 ring-blue-200' : ''}`}
    >
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500">Welcome back! Here's your engineering overview.</p>
            </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section>
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-sm text-gray-500">{analysisJobs.length} jobs</span>
              </div>
