    ...
    keyMetrics: [
      {
        ...
        unit: "kN",
        status: "ok",
        location: "Support S-12",
      },
      {
        name: "Total Base Shear X",
        value: 1856,
        unit: "kN",
        status: "ok",
        location: "All Supports",
      },
      {
        name: "Max. Moment",
        value: 324.5,
...
const MOCK_MEMBER_RESULTS: MemberResult[] = [
  {
    ...
    name: "Beam B-45",
    type: "Beam",
    utilizationRatio: 0.98,
    maxStress: 285,
    maxDeflection: 12.5,
    status: "fail",
    criticalCheck: "Combined Stress",
    section: "W24x68",
    material: "A992 Gr50",
    length: 8.5,
  },
    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          <div
            className={`p-2 rounded-lg ${isSelected ? "bg-cyan-500/20 text-cyan-400" : "bg-slate-700 text-slate-400"}`}
          >
            {summary.icon}
          </div>
          <div>
...
const MemberResultsTable: React.FC<MemberResultsTableProps> = ({
  results,
  onViewDetails,
}) => {
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    <div className="w-16 h-2 bg-slate-700 rounded-full overflow-hidden">
                      <div
                        className={`h-full ${
                          member.utilizationRatio > 1
                            ? "bg-red-500"
                            : member.utilizationRatio > 0.9
                              ? "bg-yellow-500"
                              : "bg-green-500"
                        }`}
                        style={{
                          width: `${Math.min(member.utilizationRatio * 100, 100)}%`,
                        }}
                      />
                    </div>
                    <span
...
export const ResultsDashboard: React.FC<ResultsDashboardProps> = ({
  ...
  const [selectedCategory, setSelectedCategory] =
    useState<ResultCategory | null>("utilization");
  const [viewMode, setViewMode] = useState<"grid" | "list">("grid");

  const selectedSummary = MOCK_RESULTS_SUMMARY.find(
    (s) => s.category === selectedCategory,
  );

  // Calculate overall statistics
  const overallStats = useMemo(() => {
    const totals = MOCK_RESULTS_SUMMARY.reduce(
      (acc, s) => ({
            ...
            <div className="space-y-6">
              ...
              <div className="bg-slate-800/50 rounded-xl p-6 border border-slate-700">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button aria-label="Maximize" title="Maximize" className="p-2 bg-slate-700 text-slate-400 hover:text-white rounded-lg">
