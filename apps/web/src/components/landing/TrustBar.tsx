...
export const TrustBar: FC = () => (
  <section className="py-14 border-y border-slate-200/60 dark:border-white/[0.04] bg-slate-50/50 dark:bg-slate-900/20">
    <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-12">
      <p className="text-center text-xs font-semibold uppercase tracking-[0.2em] text-soft mb-8">
      ...
      <div className="flex flex-wrap items-center justify-center gap-x-12 gap-y-6">
        {TRUST_LOGOS.map((logo, i) => (
          <span
            key={i}
            className="text-soft/70 hover:text-[var(--color-text)] transition-all duration-300 text-lg font-semibold tracking-wide cursor-default select-none"
          >
            {logo}
          </span>
        ))}
