...
const SummaryCard: React.FC<{
  ...
  <motion.div
    ...
    className={`relative overflow-hidden rounded-xl border ${critical ? 'border-red-300 bg-red-50' : 'border-gray-200 bg-white'} p-4 shadow-sm`}
  >
    {critical && (
...
const DiagramCanvas: React.FC<{
  ...
  return (
    <div className="relative">
      <svg width="100%" viewBox={`0 0 ${width} ${height}`} className="border border-gray-200 rounded-lg bg-white">
        {/* Grid */}
        <defs>
      ...
      <div className="absolute bottom-4 left-4 flex gap-4 text-sm">
        <div className="rounded bg-white/90 px-2 py-1 shadow">
      ...
      {results.slice(0, 3).map((mode) => (
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <h4 className="font-semibold text-gray-900">Mode {mode.modeNumber}</h4>
            <span className="text-xs text-gray-500">T = {formatNumber(mode.period, 3)}s</span>
          </div>
          <div className="text-3xl font-bold text-blue-600 mb-2">
            {formatNumber(mode.frequency)} <span className="text-sm font-normal text-gray-500">Hz</span>
          </div>
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              <span className="text-gray-500">Mass X:</span>
            ...
            <div className="flex justify-between text-sm">
              <span className="text-gray-500">Mass Y:</span>
      ...
      <header className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500">Load Case: {selectedLoadCase}</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div className="flex rounded-lg border border-gray-200 overflow-hidden">
              <button
                ...
                className="px-3 py-2 text-sm font-medium text-gray-600 hover:bg-gray-50"
              >
                PDF
              </button>
              <button
                onClick={() => onExport?.('excel')}
                className="px-3 py-2 text-sm font-medium text-gray-600 hover:bg-gray-50 border-l border-gray-200"
              >
                Excel
              </button>
              <button
          ...
          {viewMode === 'summary' && (
            ...
            >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className={`font-semibold ${
                      results.summary.stabilityCheck === 'stable' ? 'text-green-800' : 'text-red-800'
                    }`}>
              ...
              <div className="rounded-xl border border-gray-200 bg-white p-6">
                <h3 className="font-semibold text-gray-900 mb-4">Structure Summary</h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                  <div>
                    <p className="text-sm text-gray-500">Total Weight</p>
                    <p className="text-xl font-semibold">{formatNumber(results.summary.totalWeight)} kg</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Members</p>
                    <p className="text-xl font-semibold">{results.memberForces.filter((f, i, arr) => arr.findIndex(a => a.memberId === f.memberId) === i).length}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Nodes</p>
                    <p className="text-xl font-semibold">{results.displacements.length}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Load Cases</p>
