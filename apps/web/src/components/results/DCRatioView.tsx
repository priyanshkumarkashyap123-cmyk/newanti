          ...
          return (
            <>
              <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-3 text-center">
                ...
                <div className="text-xs text-slate-500 dark:text-slate-400">Safe (&le;70%)</div>
              </div>
              <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-3 text-center">
                <div className="text-2xl font-bold text-yellow-400">{warn}</div>
                <div className="text-xs text-slate-500 dark:text-slate-400">Warning (70-90%)</div>
              </div>
              <div className="bg-orange-500/10 border border-orange-500/30 rounded-lg p-3 text-center">
                <div className="text-2xl font-bold text-orange-400">{crit}</div>
                <div className="text-xs text-slate-500 dark:text-slate-400">Critical (90-100%)</div>
              </div>
              <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3 text-center">
                <div className="text-2xl font-bold text-red-400">{fail}</div>
                <div className="text-xs text-slate-500 dark:text-slate-400">Failed (&gt;100%)</div>
              </div>
            </>
          );
        })()}
          ...
          <table className="w-full text-sm">
            <thead className="sticky top-0 bg-white dark:bg-slate-900">
              <tr className="border-b border-slate-200 dark:border-slate-700">
                ...
                <th className="px-3 py-2 text-left text-slate-500 dark:text-slate-400">Status</th>
                <th className="px-3 py-2 text-left text-slate-500 dark:text-slate-400">Governing</th>
              </tr>
            </thead>
            <tbody>
              {[...members]
                ...
                .map((m) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-1.5 text-xs text-slate-500 dark:text-slate-400">{governing}</td>
                    </tr>
                  );
                })}
            </tbody>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-sm text-slate-900 dark:text-white">{limit.label}</div>
                    <div className="text-xs text-slate-500 dark:text-slate-400">{limit.code}</div>
                  </div>
                </div>
                <div className="text-right">
                  {allPass ? (
                    ...
                    <div>
                      <span className="text-xs text-red-400 font-medium">{violations.length} FAIL</span>
          ...
          return (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="sticky top-0 bg-white dark:bg-slate-900">
                  <tr className="border-b border-slate-200 dark:border-slate-700">
                    <th className="px-3 py-2 text-left text-slate-500 dark:text-slate-400 text-xs">Story</th>
                    <th className="px-3 py-2 text-left text-slate-500 dark:text-slate-400 text-xs">Height (mm)</th>
                    <th className="px-3 py-2 text-left text-slate-500 dark:text-slate-400 text-xs">Drift (mm)</th>
                    <th className="px-3 py-2 text-left text-slate-500 dark:text-slate-400 text-xs">Δ/H Ratio</th>
                    <th className="px-3 py-2 text-left text-slate-500 dark:text-slate-400 text-xs">H/400 (IS 1893)</th>
                    <th className="px-3 py-2 text-left text-slate-500 dark:text-slate-400 text-xs">H/500 (ASCE 7)</th>
                  </tr>
                </thead>
                <tbody>
                  {driftResults.map((dr, i) => (
                    <tr key={i} className="border-b border-slate-200 dark:border-slate-800 hover:bg-slate-200/50 dark:hover:bg-slate-800/50">
                      <td className="px-3 py-1.5 font-medium text-slate-900 dark:text-white text-xs">{dr.storyLabel}</td>
