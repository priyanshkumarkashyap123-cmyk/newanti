...
const ToggleButton: FC<ToggleButtonProps> = ({ active, onClick, children, color = '#00aaff' }) => (
    <button
        onClick={onClick}
        ...
        style={active ? { borderColor: color + '80', backgroundColor: color + '20' } : {}}
    >
        {children}
    </button>
);
...
const SummaryCard: FC<SummaryCardProps> = ({ label, value, unit, status, icon }) => {
    const statusColors = {
    ...
    return (
        <div className={`bg-slate-900 rounded-lg border border-slate-700 overflow-hidden ${className}`}>
            ...
            <div className="max-h-[calc(100vh-200px)] overflow-y-auto">
                ...
                {/* Diagram Selection */}
                <CollapsibleSection title="Force Diagrams" icon={<BarChart3 size={14} />}>
                    <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            {diagramOptions.map(opt => (
                                <button
                                    ...
                                    style={settings.showDiagram === opt.id 
                                        ? { borderColor: opt.color + '80', backgroundColor: opt.color + '15' }
                                        : {}}
                                    title={opt.description}
                                >
                                    <div style={{ color: settings.showDiagram === opt.id ? opt.color : undefined }}>
                                        {opt.icon}
                                    </div>
                        ...
                        {/* Scale Control */}
                        {settings.showDiagram !== 'none' && settings.showDiagram !== 'DEFLECTION' && (
                            <ScaleSlider
                                label="Diagram Scale"
                        ...
                        {settings.showDiagram !== 'none' && (
                            <div className="flex flex-wrap gap-2">
                                ...
                                <ToggleButton
                                    ...
                                    onClick={() => updateSettings({ showCriticalPoints: !settings.showCriticalPoints })}
                                >
                                    Critical Pts
                                </ToggleButton>
                                <ToggleButton
                                    active={settings.showFill}
                                    onClick={() => updateSettings({ showFill: !settings.showFill })}
                                >
                                    Fill
                                </ToggleButton>
                                <ToggleButton
                                    active={settings.showBaseline}
                                    onClick={() => updateSettings({ showBaseline: !settings.showBaseline })}
                                >
                                    Baseline
                                </ToggleButton>
                            </div>
                ...
                {/* Color Mode */}
                <CollapsibleSection title="Color Scheme" icon={<Palette size={14} />} defaultOpen={false}>
                    <div className="space-y-2">
                        {[
                            { id: 'default', label: 'Standard Colors', desc: 'Classic diagram colors' },
                            { id: 'stress', label: 'Stress Gradient', desc: 'Blue-White-Red stress scale' },
                            { id: 'utilization', label: 'Utilization', desc: 'Green-Yellow-Red capacity' },
                            { id: 'force', label: 'Force Intensity', desc: 'Color by force magnitude' },
                        ].map(mode => (
                            <label
                                key={mode.id}
                            ...
                            >
                                <input
                                    ...
                                    onChange={() => updateSettings({ colorMode: mode.id as ColorMode })}
                ...
                {/* Animation Controls */}
                <CollapsibleSection title="Animation" icon={<Play size={14} />} defaultOpen={false}>
                    <div className="space-y-3">
                        <div className="flex items-center justify-between">
                            <span className="text-sm text-slate-400">Animate Deflection</span>
                            <button
                                onClick={() => updateSettings({ animateDeflection: !settings.animateDeflection })}
                                className={`w-12 h-6 rounded-full transition-colors ${
                                    settings.animateDeflection ? 'bg-cyan-600' : 'bg-slate-600'
                                }`}
                            >
                                <div className={`w-5 h-5 rounded-full bg-white shadow-md transform transition-transform ${
                                    settings.animateDeflection ? 'translate-x-6' : 'translate-x-0.5'
                                }`} />
                            </button>
                        </div>
                ...
                {/* View Options */}
                <CollapsibleSection title="View Options" icon={<Eye size={14} />} defaultOpen={false}>
                    <div className="space-y-2">
                        <label className="flex items-center gap-2 text-sm text-slate-300 cursor-pointer">
                            <input
                                type="checkbox"
                                checked={settings.showOriginalShape}
                                onChange={(e) => updateSettings({ showOriginalShape: e.target.checked })}
                ...
                {/* Model Info */}
                <CollapsibleSection title="Model Info" icon={<Layers size={14} />} defaultOpen={false}>
