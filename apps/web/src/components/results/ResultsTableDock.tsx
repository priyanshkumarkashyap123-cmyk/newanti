  ...
  ({ analysisResults, onClose }) => {
    ...
    const SortHeader: FC<{ col: string; label: string }> = ({ col, label }) => (
      <th
        ...
        className="px-3 py-2 text-left text-[10px] font-bold uppercase tracking-wider text-[#869ab8] cursor-pointer hover:text-blue-400 transition-colors select-none whitespace-nowrap"
      >
    ...
    if (isCollapsed) {
      return (
        <div className="h-7 bg-white/95 dark:bg-slate-900/95 backdrop-blur-sm border-t border-[#1a2333]/40 flex items-center justify-between px-3 flex-shrink-0">
          <div className="flex items-center gap-4 text-[11px] font-mono">
            <span className="font-semibold text-slate-400">Results</span>
            <span className="text-slate-500">
              Nodes: <span className="text-[#adc6ff]">{displacementRows.length}</span>
            </span>
            <span className="text-slate-500">
              Members: <span className="text-[#adc6ff]">{memberForceRows.length}</span>
            </span>
            <span className="text-slate-500">
              Reactions: <span className="text-[#adc6ff]">{reactionRows.length}</span>
            </span>
            {equilibriumOk !== null && (
              <span
    ...
    return (
      <div
        className="h-[240px] flex flex-col bg-white/98 dark:bg-slate-900/98 backdrop-blur-md border-t border-[#1a2333]/40 flex-shrink-0 transition-all duration-200 animate-[slideInUp_200ms_ease-out]"
      >
        {/* Tab Header Bar */}
        <div className="flex items-center justify-between h-8 px-2 bg-slate-100/80 dark:bg-slate-950/80 border-b border-[#1a2333]/30 flex-shrink-0">
          <div className="flex items-center gap-0.5">
            {tabs.map((tab) => {
              ...
              >
                {tab.label}
                <span className="ml-1.5 px-1.5 py-0.5 rounded text-[9px] bg-slate-200 dark:bg-slate-700/50 text-[#869ab8]">
                  {tab.count}
                </span>
              </button>
              );
          ...
          <div className="flex items-center gap-1">
            ...
            <div className="flex items-center gap-1 mr-2">
              ...
              <input
                ...
                className="w-24 h-5 px-1.5 text-[10px] bg-[#131b2e] border border-[#1a2333]/40 rounded text-[#adc6ff] placeholder:text-slate-400 dark:placeholder:text-slate-600 focus:border-blue-500/50 focus:outline-none"
              />
            </div>
            <button type="button"
        ...
        <div className="flex-1 overflow-auto eng-scroll">
          <table className="w-full text-[11px] font-mono">
              ...
              {activeTab === "displacements" &&
                sortRows(displacementRows).map((r) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-1.5 text-[#adc6ff] tabular-nums">
                      {formatNum(r.dx)}
                    </td>
                    <td className="px-3 py-1.5 text-[#adc6ff] tabular-nums">
                      {formatNum(r.dy)}
                    ...
                    <td className="px-3 py-1.5 text-[#869ab8] tabular-nums">
                      {formatNum(r.rx, 6)}
                    </td>
                    <td className="px-3 py-1.5 text-[#869ab8] tabular-nums">
                      {formatNum(r.ry, 6)}
            ...
            {activeTab === "reactions" && reactionRows.length > 0 && (
              <tfoot className="sticky bottom-0 bg-slate-100/95 dark:bg-slate-950/95 backdrop-blur-sm border-t border-[#1a2333]/40">
