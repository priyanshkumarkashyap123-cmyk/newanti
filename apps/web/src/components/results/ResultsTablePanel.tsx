...
function TableHeader<T>({ columns, sortColumn, sortDirection, onSort }: TableHeaderProps<T>) {
    return (
...
const NodeResultsTable: FC<NodeResultsTableProps> = ({ searchQuery }) => {
    ...
    const [sortColumn, setSortColumn] = useState<keyof NodeResultRow | null>(null);
    ...
    const handleSort = useCallback((column: keyof NodeResultRow) => {
                ...
                <tbody>
                    {sortedData.map((row, idx) => {
                        ...
                        return (
                            ...
                            >
                                <td className="px-3 py-1.5 text-left font-mono text-slate-400">{row.nodeId}</td>
                                ...
                                <td className="px-3 py-1.5 text-right font-mono text-cyan-400">{formatNumber(row.dx)}</td>
                                <td className="px-3 py-1.5 text-right font-mono text-cyan-400">{formatNumber(row.dy)}</td>
                                <td className="px-3 py-1.5 text-right font-mono text-cyan-400">{formatNumber(row.dz)}</td>
                                <td className="px-3 py-1.5 text-right font-mono text-amber-400">{formatNumber(row.rx, 6)}</td>
                                <td className="px-3 py-1.5 text-right font-mono text-amber-400">{formatNumber(row.ry, 6)}</td>
                                <td className="px-3 py-1.5 text-right font-mono text-amber-400">{formatNumber(row.rz, 6)}</td>
                                <td className={`px-3 py-1.5 text-right font-mono font-bold ${isMax ? 'text-red-400' : 'text-emerald-400'}`}>
...
const MemberResultsTable: FC<MemberResultsTableProps> = ({ searchQuery }) => {
    ...
    const [sortColumn, setSortColumn] = useState<keyof MemberResultRow | null>(null);
    const [sortDirection, setSortDirection] = useState<SortDirection>(null);
    ...
    const data = useMemo<MemberResultRow[]>(() => {
        ...
        members.forEach((member, memberId) => {
            ...
            if (forces && startNode && endNode) {
                ...
                const utilizationRatio = Math.min((maxMoment * 0.001 + Math.abs(forces.axial) * 0.0005) * 100, 150);
                ...
                rows.push({
                    ...
                    length,
                    axialStart: forces.axial,
                    axialEnd: forces.axial,
                    shearYStart: forces.shearY,
                    shearYEnd: -forces.shearY,
                    shearZStart: forces.shearZ,
                    shearZEnd: -forces.shearZ,
                    momentYStart: forces.momentY,
                    momentYEnd: -forces.momentY,
                    momentZStart: forces.momentZ,
                    momentZEnd: -forces.momentZ,
    ...
    const handleSort = useCallback((column: keyof MemberResultRow) => {
        if (sortColumn === column) {
            setSortDirection(prev => prev === 'asc' ? 'desc' : prev === 'desc' ? null : 'asc');
            if (sortDirection === 'desc') setSortColumn(null);
        } else {
                    ...
                    {sortedData.map((row, idx) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <td className="px-3 py-1.5 text-right font-mono text-cyan-400">{formatNumber(row.shearYStart)}</td>
                                <td className="px-3 py-1.5 text-right font-mono text-cyan-400">{formatNumber(row.shearZStart)}</td>
                                <td className="px-3 py-1.5 text-right font-mono text-amber-400">{formatNumber(row.momentYStart)}</td>
                                <td className="px-3 py-1.5 text-right font-mono text-amber-400">{formatNumber(row.momentZStart)}</td>
                                <td className="px-3 py-1.5 text-right font-mono text-purple-400">{formatNumber(row.torsion)}</td>
                                <td className="px-3 py-1.5 text-right font-mono font-bold text-cyan-300">{formatNumber(row.maxShear)}</td>
                                <td className="px-3 py-1.5 text-right font-mono font-bold text-amber-300">{formatNumber(row.maxMoment)}</td>
                                <td className={`px-3 py-1.5 text-right font-mono font-bold ${
                                    isOverUtilized ? 'text-red-400' : isWarning ? 'text-amber-400' : 'text-emerald-400'
                                }`}>
                                    {row.utilizationRatio.toFixed(1)}
                                    {isOverUtilized && <AlertTriangle size={12} className="inline ml-1" />}
                                    {!isOverUtilized && !isWarning && <CheckCircle size={12} className="inline ml-1" />}
                                </td>
                            </tr>
                        );
                    })}
...
const ReactionsTable: FC<ReactionsTableProps> = ({ searchQuery }) => {
    ...
    const handleSort = useCallback((column: keyof ReactionRow) => {
        if (sortColumn === column) {
            setSortDirection(prev => prev === 'asc' ? 'desc' : prev === 'desc' ? null : 'asc');
            if (sortDirection === 'desc') setSortColumn(null);
        } else {
                ...
                <tbody>
                    {sortedData.map((row, idx) => (
                        ...
                        >
                            ...
                            <td className="px-3 py-1.5 text-right font-mono text-amber-400">{formatNumber(row.My)}</td>
                            <td className="px-3 py-1.5 text-right font-mono text-amber-400">{formatNumber(row.Mz)}</td>
...
export const ResultsTablePanel: FC<ResultsTablePanelProps> = ({
    ...
    const tabs: { id: ResultsTab; label: string; icon: React.ReactNode }[] = [
        { id: 'nodes', label: 'Nodes', icon: <div className="w-2 h-2 rounded-full bg-cyan-400" /> },
        { id: 'members', label: 'Members', icon: <div className="w-4 h-0.5 bg-amber-400 rounded" /> },
        { id: 'reactions', label: 'Reactions', icon: <div className="w-2 h-2 bg-green-400 rotate-45" /> },
    ];
    ...
    return (
        <div className={`bg-slate-900 rounded-lg border border-slate-700 overflow-hidden ${className}`}>
            ...
            <div className="overflow-x-auto">
                {activeTab === 'nodes' && <NodeResultsTable searchQuery={searchQuery} />}
                {activeTab === 'members' && <MemberResultsTable searchQuery={searchQuery} />}
                {activeTab === 'reactions' && <ReactionsTable searchQuery={searchQuery} />}
            </div>
                ...
                <div className="px-4 py-2 border-t border-slate-700 bg-slate-800/30 text-xs text-slate-500">
                    <div className="flex justify-between">
                        <span>
                            ...
                            {activeTab === 'members' && `${analysisResults.memberForces.size} members analyzed`}
                            {activeTab === 'reactions' && `${analysisResults.reactions.size} support reactions`}
                        </span>
                        <span>
                            Analysis completed in {((analysisResults as any).analysisTime || 0).toFixed(2)}ms
                        </span>
                    </div>
                </div>
            )}
