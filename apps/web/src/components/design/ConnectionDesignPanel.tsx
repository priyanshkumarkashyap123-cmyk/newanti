...
export const ConnectionDesignPanel: React.FC = () => {
    ...
    return (
        <div className="space-y-6 p-6 bg-zinc-900 min-h-screen text-white">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {/* Input Panel */}
                <div className="space-y-4 bg-zinc-800/50 p-6 rounded-xl border border-zinc-700">
                    <h3 className="text-lg font-semibold text-orange-300">Connection Parameters</h3>

                    <div>
                        <label className="block text-sm text-zinc-400 mb-1">Connection Type</label>
                        <select
                            value={connType}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm text-zinc-400 mb-1">Shear Load (kN)</label>
                            <input
                                type="number"
                        ...
                        {(connType !== 'shear_bolt') && (
                            <>
                                <div>
                                    <label className="block text-sm text-zinc-400 mb-1">Moment (kNm)</label>
                                    <input
                                        type="number"
                                ...
                                <div>
                                    <label className="block text-sm text-zinc-400 mb-1">Axial (kN)</label>
                                    <input
                                        type="number"
                        ...
                        {(connType !== 'base_plate') && (
                            <div>
                                <label className="block text-sm text-zinc-400 mb-1">Beam Depth (mm)</label>
                                <input
                                    type="number"
                        ...
                        {(connType === 'base_plate') && (
                            <>
                                <div>
                                    <label className="block text-sm text-zinc-400 mb-1">Plate Width (mm)</label>
                                    <input
                                        type="number"
                                ...
                                <div>
                                    <label className="block text-sm text-zinc-400 mb-1">Plate Length (mm)</label>
                                    <input
                                        type="number"
                        ...
                        <div>
                            <label className="block text-sm text-zinc-400 mb-1">Bolt Dia (mm)</label>
                            <select
                ...
                <div className="bg-zinc-800/50 p-6 rounded-xl border border-zinc-700">
                    ...
                    {result ? (
                        <div className="space-y-4">
                            <div className="p-3 bg-zinc-900 rounded border border-zinc-700 text-sm font-mono whitespace-pre-wrap">
                                {result.connection}
                            </div>
                            ...
                            <div className="space-y-1">
                                <h4 className="text-sm font-medium text-zinc-400">Detailed Checks</h4>
                                {result.checks.map((check, i) => (
                                    <div key={i} className="text-xs text-zinc-400 p-2 bg-zinc-900/50 rounded border border-zinc-800">
                                        {check}
                                    </div>
                                ))}
                            </div>
