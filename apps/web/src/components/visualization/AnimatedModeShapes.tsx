...
export const AnimatedModeShapes: React.FC<{
     ...
     */
    const getDeformedPosition = useCallback((nodeId: string, t: number): NodePosition | null => {
        if (!currentMode) return null;

        const originalNode = nodes.find(n => n.id === nodeId);
        const displacement = currentMode.displacements.find(d => d.nodeId === nodeId);
    ...
    }, [currentMode, nodes, settings.amplitudeScale, settings.speed]);
    ...
    const draw = useCallback((t: number) => {
        ...
        if (currentMode) {
            ...
            for (const member of members) {
                ...
                const disp = currentMode.displacements.find(d => d.nodeId === member.startNode);
            ...
            for (const node of nodes) {
                ...
                const disp = currentMode.displacements.find(d => d.nodeId === node.id);
            ...
            <div className="px-4 py-3 bg-gray-800 border-b border-gray-700 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    <button
                        ...
                        className={`px-3 py-1 rounded text-sm font-medium ${isPlaying ? 'bg-yellow-500 text-black' : 'bg-green-500 text-white'
                            }`}
                    >
            ...
            <div className="p-4 bg-gray-800 border-t border-gray-700 space-y-3">
                    ...
                    <div className="flex gap-1">
                        {modeShapes.slice(0, 10).map((mode, i) => (
                            <button
                                ...
                                className={`w-8 h-8 rounded text-sm font-medium ${selectedMode === i
                ...
                <div className="flex items-center gap-4">
                    ...
                    <input
                        ...
                        onChange={(e) => setSettings(s => ({ ...s, amplitudeScale: Number(e.target.value) }))}
                        className="flex-1"
                    />
                    <span className="text-white text-sm w-10">{settings.amplitudeScale}x</span>
                </div>
                ...
                <div className="flex items-center gap-6">
                    <label className="flex items-center gap-2 text-sm text-gray-300">
                        <input
                            ...
                            onChange={(e) => setSettings(s => ({ ...s, showUndeformed: e.target.checked }))}
                            className="rounded"
                        />
                        Show undeformed
                    </label>
                    <label className="flex items-center gap-2 text-sm text-gray-300">
                        <select
                            ...
                            onChange={(e) => setSettings(s => ({ ...s, colorScale: e.target.value as any }))}
