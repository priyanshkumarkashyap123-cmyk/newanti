...
const ToolButton = memo<ToolButtonProps>(
  ({
    ...
    <Tooltip content={tooltip || label} shortcut={shortcut}>
      ...
      >
        <Icon
          className={`${vertical ? "w-4.5 h-4.5" : "w-4 h-4"} flex-shrink-0`}
        />
...
const ToolGroup = memo<{ label: string; children: ReactNode }>(
  ({ label, children }) => (
...
export const EngineeringRibbon: FC<RibbonProps> = memo(({ activeCategory }) => {
    ...
    <>
      <ToolGroup label="Project">
        ...
        <ToolButton
          ...
          label="Open"
          onClick={() =>
            document.dispatchEvent(new CustomEvent("trigger-cloud-open"))
          }
          shortcut="Cmd+O"
        />
        <div className="flex flex-col gap-0.5">
          <Tooltip content="Undo" shortcut="Cmd+Z">
            <button
              onClick={() => undo()}
              className="p-1 hover:bg-slate-700/60 rounded text-slate-400"
              aria-label="Undo"
            >
          ...
          <Tooltip content="Redo" shortcut="Cmd+Shift+Z">
            <button
              ...
              aria-label="Redo"
            >
              <Redo className="w-3 h-3" aria-hidden="true" />
            </button>
          </Tooltip>
        </div>
        <ToolButton
      ...
      <ToolGroup label="Structure">
        ...
        <ToolButton
          ...
          tooltip="Load Famous Structures"
        />
      </ToolGroup>
      <ToolGroup label="Create">
        ...
        <ToolButton
          ...
          tooltip="Add Floor Slab — auto-detect panels & assign area load"
        />
      </ToolGroup>
    ...
    <>
      <ToolGroup label="Simulation">
        ...
        <ToolButton
          ...
          label="Modal"
          onClick={() =>
            document.dispatchEvent(new CustomEvent("trigger-modal-analysis"))
          }
        />
      </ToolGroup>
  ...
  return (
    <div
      className="w-full bg-slate-900/95 backdrop-blur-sm border-b border-slate-700/60 flex flex-col select-none"
      role="toolbar"
    ...
    >
      ...
      <div className="h-9 flex items-center justify-between px-3 border-b border-slate-700/40 bg-slate-950/80">
        ...
        >
          ...
          <span className="px-1 py-0.5 bg-blue-500/10 border border-blue-500/20 text-blue-400 text-[8px] font-bold rounded tracking-wide">
            PRO
          </span>
        </Link>
      ...
      {/* Tools Area */}
      <div
        className="h-[72px] flex items-center px-1.5 py-0.5 gap-0.5 overflow-x-auto"
        role="group"
          ...
          <>
            <ToolGroup label="Code Check">
              <ToolButton
                ...
                tooltip="Open Design Code Compliance Dialog"
              />
              <ToolButton
                icon={CheckSquare}
                label="D/C Ratios"
                ...
                tooltip="Run Analysis & View D/C Ratios"
              />
            </ToolGroup>
            <ToolGroup label="Steel">
              <ToolButton
                ...
                tooltip="Steel Design Studio — IS 800 / AISC 360 / EN 1993"
              />
            </ToolGroup>
            <ToolGroup label="Concrete">
              <ToolButton
                ...
                tooltip="RC Design Studio — IS 456 / ACI 318 / EN 1992"
              />
            </ToolGroup>
            <ToolGroup label="Connection">
              <ToolButton
                ...
                tooltip="Connection Design — Bolted/Welded"
              />
            </ToolGroup>
            <ToolGroup label="Foundation">
              <ToolButton
                ...
                tooltip="Foundation Design — IS 456 / IS 1904"
              />
            </ToolGroup>
            <ToolGroup label="Advanced">
              <ToolButton
                ...
                tooltip="Detailed Section Design"
              />
              <ToolButton
                icon={Globe}
                label="Curved"
                onClick={() => openModal("curvedStructure")}
                tooltip="Curved Structure Generator"
              />
            </ToolGroup>
          </>
        )}
          ...
          <>
            <ToolGroup label="Civil Engineering">
              ...
              <ToolButton
                ...
                label="Geotech"
                onClick={() => openModal("civilEngineering")}
                tooltip="Geotechnical Engineering — Bearing Capacity, Settlement"
              />
              <ToolButton
                icon={Droplets}
                label="Hydraulics"
                onClick={() => openModal("civilEngineering")}
                tooltip="Hydraulics — Open Channel Flow, Pipe Networks"
              />
            </ToolGroup>
            <ToolGroup label="Infrastructure">
              <ToolButton
                ...
                label="Transport"
                onClick={() => openModal("civilEngineering")}
                tooltip="Transportation — Highway Design, Pavement, Traffic"
              />
              <ToolButton
                icon={HardHat}
                label="Construction"
                onClick={() => openModal("civilEngineering")}
                tooltip="Construction Management — CPM/PERT, Gantt"
              />
            </ToolGroup>
            <ToolGroup label="Design Tools">
              <ToolButton
                icon={Building2}
                label="Steel"
                onClick={() => openModal("steelDesign")}
                tooltip="Steel Design Studio — IS 800 / AISC 360"
              />
              <ToolButton
                icon={Columns}
                label="Concrete"
                onClick={() => openModal("concreteDesign")}
                tooltip="Concrete Design — IS 456 / ACI 318"
              />
              <ToolButton
                icon={Link2}
                label="Connect"
            ...
            <ToolGroup label="Advanced">
              <ToolButton
                ...
                tooltip="Generative Design / Topology Optimization"
              />
              <ToolButton
                icon={BarChart3}
                label="Seismic"
                onClick={() => openModal("seismicStudio")}
                tooltip="Seismic Design Studio"
              />
            </ToolGroup>
          </>
        )}
