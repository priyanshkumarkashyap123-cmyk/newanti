        ...
        <div className="bg-[#0b1326] rounded-xl border border-slate-700 overflow-hidden">
            ...
            <div className="px-4 py-3 bg-[#131b2e] border-b border-slate-700 flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-semibold text-[#dae2fd]">AI Self-Improvement</h3>
                    {isMonitoring && (
                ...
                <div className="grid grid-cols-5 gap-3">
                    {metrics.map(m => (
                        <div
                            ...
                            className={`bg-[#131b2e] rounded-lg p-3 cursor-pointer transition-colors ${selectedFeature === m.feature ? 'ring-2 ring-purple-500' : 'hover:bg-slate-750'
                                }`}
                        >
                            <div className="flex items-center justify-between mb-1">
                                <span className="text-[#869ab8] text-xs capitalize">{m.feature.replace('_', ' ')}</span>
                    ...
                    <div className="bg-[#131b2e] rounded-lg p-4">
                        ...
                        <div className="flex gap-4 text-sm">
                            <div>
                                ...
                                <span className="ml-2 text-[#dae2fd] font-bold">{knowledgeStats.nodeCount}</span>
                            </div>
                            <div>
                                <span className="text-[#869ab8]">Edges:</span>
                                <span className="ml-2 text-[#dae2fd] font-bold">{knowledgeStats.edgeCount}</span>
                            </div>
                            {Object.entries(knowledgeStats.byType).map(([type, count]) => (
                                <div key={type}>
                                    <span className="text-[#869ab8] capitalize">{type}:</span>
                                    <span className="ml-1 text-[#dae2fd]">{count as number}</span>
                                </div>
                            ))}
                        </div>
                    </div>
