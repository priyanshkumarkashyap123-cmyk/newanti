...
export const AdvancedAIBrain: FC = () => {
    ...
    return (
      ...
      >
        ...
        <motion.span
          ...
          className="absolute right-full mr-3 px-3 py-1.5 bg-slate-800 text-white text-sm font-medium rounded-lg whitespace-nowrap"
        >
          AI Assistant
        </motion.span>
      </motion.button>
  ...
  return (
    <motion.div
      ...
      className={`fixed z-50 bg-slate-900 border border-slate-700/50 shadow-2xl ${
        isFullscreen 
          ? 'inset-4 rounded-2xl' 
          ...
          <div className="flex flex-wrap gap-2">
            {QUICK_PROMPTS.map((prompt, i) => (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs bg-slate-800 text-slate-300 rounded-full hover:bg-slate-700 transition-colors"
              >
                {prompt.icon}
                {prompt.text}
              </button>
        ...
        <div className="flex items-end gap-2">
          <div className="flex-1 relative">
            <textarea
              ...
              rows={1}
              className="w-full px-4 py-3 pr-24 bg-slate-800 text-white placeholder-slate-500 rounded-xl resize-none outline-none focus:ring-2 focus:ring-blue-500/50 transition-all"
