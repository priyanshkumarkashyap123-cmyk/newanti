...
export function useMultiplayer(config: MultiplayerConfig) {
  ...
  useEffect(() => {
    // Create socket connection with limited retries to avoid console spam
    ...
    socket.on(
      "project_joined",
      (data: { projectId: string; users: RemoteUser[]; version: number }) => {
    ...
    socket.on(
      "analysis_complete",
      (data: { results: any; userName: string }) => {
    ...
    // Handle connection errors gracefully (server may not be running)
    socket.on("connect_error", () => {
      // Silently stay in demo/offline mode — no spam
      setState((prev) => ({ ...prev, isConnected: false }));
    });
  ...
  const updateNode = useCallback(
    (nodeId: string, updates: Partial<NodeUpdate>) => {
  ...
  const updateMember = useCallback(
    (memberId: string, updates: Partial<MemberUpdate>) => {
      if (!socketRef.current?.connected) return;
