...
export interface AIHookActions {
  ...
  generateStructure: (prompt: string, constraints?: Record<string, any>) => Promise<AIResponse & { structure?: NormalizedStructureData }>;

  /** Check design code compliance */
  checkCompliance: (member: Record<string, any>, forces: Record<string, any>, code?: string) => Promise<AIResponse>;
  ...
  autoFix: (modelData: Record<string, any>, issues: string[]) => Promise<AIResponse>;

  /** Modify model via natural language */
  modifyModel: (modelData: Record<string, any>, instruction: string) => Promise<AIResponse>;
  ...
  autonomousDesign: (prompt: string, options?: Partial<DesignRequest>) => Promise<DesignResult>;

  /** Quick design check without optimization */
  quickDesignCheck: (prompt: string) => Promise<{ passed: boolean; maxUtilization: number; weight: number; report: string; failedMembers: string[] }>;
  ...
  recordFeedback: (requestId: string, rating: 1 | 2 | 3 | 4 | 5, comment?: string) => Promise<void>;
...
export function useAIOrchestrator(): UseAIOrchestratorReturn {
  ...
  const [status, setStatus] = useState<ReturnType<typeof aiOrchestrator.getStatus> | null>(null);
  ...
  useEffect(() => {
    ...
    const unsubCircuitOpen = aiOrchestrator.on('circuit-opened', (event) => {
      console.warn(`[useAI] Circuit opened for provider: ${event.data?.provider}`);
      refreshStatus();
    });
  ...
  const wrapCall = useCallback(async <T extends AIResponse>(fn: () => Promise<T>): Promise<T> => {
    setLoading(true);
    setError(null);
  ...
  const chat = useCallback(async (message: string, context?: string) => {
    const response = await wrapCall(() => aiOrchestrator.chat(message, context));
    setMessageCount(prev => prev + 1);
    return response;
  }, [wrapCall]);

  const generateStructure = useCallback(async (prompt: string, constraints?: Record<string, any>) => {
    return wrapCall(() => aiOrchestrator.generateStructure(prompt, constraints));
  }, [wrapCall]);

  const checkCompliance = useCallback(async (member: Record<string, any>, forces: Record<string, any>, code?: string) => {
    return wrapCall(() => aiOrchestrator.checkCompliance(member, forces, code));
  }, [wrapCall]);

  const explain = useCallback(async (topic: string) => {
    return wrapCall(() => aiOrchestrator.explain(topic));
  }, [wrapCall]);

  const diagnoseModel = useCallback(async (modelData: Record<string, any>) => {
    return wrapCall(() => aiOrchestrator.diagnoseModel(modelData));
  }, [wrapCall]);

  const autoFix = useCallback(async (modelData: Record<string, any>, issues: string[]) => {
    return wrapCall(() => aiOrchestrator.autoFix(modelData, issues));
  }, [wrapCall]);

  const modifyModel = useCallback(async (modelData: Record<string, any>, instruction: string) => {
    return wrapCall(() => aiOrchestrator.modifyModel(modelData, instruction));
  }, [wrapCall]);

  const analyzeSketch = useCallback(async (imageData: string) => {
    return wrapCall(() => aiOrchestrator.analyzeSketch(imageData));
  }, [wrapCall]);

  const autonomousDesign = useCallback(async (prompt: string, options?: Partial<DesignRequest>) => {
    setLoading(true);
    setError(null);
    try {
      ...
      const message = err instanceof Error ? err.message : 'Autonomous design failed';
      setError(message);
      throw err;
    } finally {
      setLoading(false);
  ...
  const quickDesignCheck = useCallback(async (prompt: string) => {
    setLoading(true);
    setError(null);
    try {
  ...
  const process = useCallback(async (request: AIRequest) => {
  ...
  const streamChat = useCallback((message: string) => {
    ...
    abortRef.current = () => { /* stream will complete or error */ };
    return streamState;
  }, [refreshStatus]);
  ...
  const recordFeedback = useCallback(async (requestId: string, rating: 1 | 2 | 3 | 4 | 5, comment?: string) => {
