...
async function loadWasmModule(): Promise<void> {
  try {
    ...
    console.warn(
      "[SolverWorker] WASM module not available, using JS fallback:",
      error,
    );
    wasmReady = false;
  }
  ...
  if (useWasm && wasmModule) {
    try {
      ...
      if (method === "cholesky") {
        ...
        displacements = wasmModule.solve_system(stiffness, forces, dof);
      }
      ...
      console.warn(
        "[SolverWorker] WASM solve failed, falling back to JS:",
        error,
      );
    }
  }
...
function solveJS(K: Float64Array, F: Float64Array, n: number): SolveResult {
  ...
  for (let k = 0; k < n - 1; k++) {
    // Find pivot
    let maxVal = Math.abs(A[k * n + k] || 0);
    let maxRow = k;

    for (let i = k + 1; i < n; i++) {
      const absVal = Math.abs(A[i * n + k] || 0);
      if (absVal > maxVal) {
        maxVal = absVal;
        maxRow = i;
      }
    ...
    // Eliminate
    const pivot = A[k * n + k] || 1; // Avoid division by zero if undefined
    for (let i = k + 1; i < n; i++) {
      const factor = (A[i * n + k] || 0) / pivot;
      for (let j = k; j < n; j++) {
        if (A[i * n + j] !== undefined) {
          A[i * n + j]! -= factor * (A[k * n + j] || 0);
        }
      }
      if (b[i] !== undefined) {
        b[i]! -= factor * (b[k] || 0);
