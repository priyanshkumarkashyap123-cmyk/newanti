...
export function analyzeDeterminacy(
    ...
    const { restrainedDOF, reactionComponents } = countRestraints(nodes, structureType);
    ...
    if (structureType === '2D') {
        if (isTruss) {
            ...
            // 2D Frame: DSI = (m + r) - 2j - c
            // Each member contributes 3 unknowns (2 moments + 1 axial)
            // Each joint provides 2 equilibrium equations (ΣFx = 0, ΣFy = 0)
            // Releases reduce indeterminacy
            DSI = (3 * m) + restrainedDOF - (2 * n) - internalReleases;
        }
    } else {
        // 3D Analysis
        if (isTruss) {
            ...
            // 3D Frame: DSI = (m + r) - 3j - c
            // Each member contributes 6 unknowns (3 forces + 3 moments at each end = 12 total, but 6 independent)
            // Each joint provides 3 equilibrium equations (ΣFx = ΣFy = ΣFz = 0)
            DSI = (6 * m) + restrainedDOF - (3 * n) - internalReleases;
        }
    }
    ...
    const hasProperArrangement = checkConstraintArrangement(nodes, members, structureType);
    ...
    if (!result.isAnalyzable) {
        ...
        result.recommendations.push('Use Direct Stiffness Method or Matrix Analysis for indeterminate structures');
    } else if (DSI === 0) {
        result.recommendations.push('Can use either Equilibrium Method or Stiffness Method');
    }
 ...
 */
function countRestraints(nodes: Node[], structureType: StructureType): {
    restrainedDOF: number;
    ...
    nodes.forEach(node => {
        ...
        if (r.fx) { count++; components.push(`${node.id}-Fx`); }
        if (r.fy) { count++; components.push(`${node.id}-Fy`); }
        if (r.mz) { count++; components.push(`${node.id}-Mz`); }
        
        if (structureType === '3D') {
            if (r.fz) { count++; components.push(`${node.id}-Fz`); }
            if (r.mx) { count++; components.push(`${node.id}-Mx`); }
            if (r.my) { count++; components.push(`${node.id}-My`); }
        }
    });
 ...
 */
function countInternalReleases(members: Member[], structureType: StructureType): number {
    let count = 0;
 ...
 */
function identifyRigidBodyModes(nodes: Node[], structureType: StructureType): string[] {
    const modes: string[] = [];
    ...
    if (structureType === '3D' && !hasZRestraint) modes.push('Rigid body translation in Z-direction');
    if (!hasRotationRestraint) modes.push(`Rigid body rotation about ${structureType === '2D' ? 'Z-axis' : 'global axes'}`);
...
function checkConstraintArrangement(
    ...
    const supportNodes = nodes.filter(n => n.restraints && 
    ...
    if (supportNodes.length === 0) {
        return {
            ...
            recommendations: ['Add at least one support to prevent rigid body motion']
        };
    }
    ...
    if (structureType === '2D' && supportNodes.length >= 2) {
        ...
        if (points.length >= 3) {
            ...
            if (areCollinear) {
                recommendations.push('Supports are collinear - may cause rotational instability');
                recommendations.push('Add a support not on the same line');
            }
        }
    }
    
    return { valid: true, message: 'Support arrangement is acceptable', recommendations };
}
...
function checkParallelSupports(
    ...
    if (nodes.length === onlyXSupports) {
        return {
            valid: false,
            message: 'All supports only restrain X-direction - structure can move in Y',
    ...
    if (nodes.length === onlyYSupports) {
        return {
            valid: false,
            message: 'All supports only restrain Y-direction - structure can move in X',
    ...
    if (structureType === '3D' && nodes.length === onlyZSupports) {
        return {
            valid: false,
            message: 'All supports only restrain Z-direction - structure can move in XY plane',
            recommendations: ['Add supports restraining XY-plane movement']
        };
    }
    
    return { valid: true, message: 'Support directions are acceptable', recommendations };
}
...
export function getDeterminacyDescription(result: DeterminacyResult): string {
    ...
    lines.push('');
    lines.push(`Nodes: ${result.numNodes} | Members: ${result.numMembers} | Reactions: ${result.numReactions}`);
    lines.push('');
