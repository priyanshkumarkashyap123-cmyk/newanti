...
// Types
type ElementType = 'quadrilateral' | 'triangular' | 'hexahedral' | 'tetrahedral' | 'shell' | 'beam';
type MeshMethod = 'mapped' | 'free' | 'adaptive' | 'multizone';
type RefinementType = 'uniform' | 'curvature' | 'proximity' | 'gradient' | 'custom';
...
const AdvancedMeshingDashboard: React.FC = () => {
  ...
  const [regions] = useState<MeshRegion[]>([
    { id: 'r1', name: 'Floor Slabs', type: 'surface', elementType: 'shell', localSize: 100, elements: 12450, quality: 92, status: 'good' },
    { id: 'r2', name: 'Columns', type: 'volume', elementType: 'beam', localSize: 50, elements: 890, quality: 95, status: 'good' },
    { id: 'r3', name: 'Beams', type: 'volume', elementType: 'beam', localSize: 75, elements: 3420, quality: 88, status: 'good' },
    { id: 'r4', name: 'Shear Walls', type: 'surface', elementType: 'shell', localSize: 150, elements: 8760, quality: 78, status: 'warning' },
    { id: 'r5', name: 'Foundation', type: 'volume', elementType: 'hexahedral', localSize: 200, elements: 5670, quality: 85, status: 'good' },
    { id: 'r6', name: 'Connection Zones', type: 'volume', elementType: 'tetrahedral', localSize: 25, elements: 14488, quality: 72, status: 'warning' },
  ]);
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-800/50 rounded-xl p-5 border border-slate-700/50">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm text-slate-300 mb-2">Element Type</label>
                  <select
                    value={settings.elementType}
                    onChange={(e) => setSettings({ ...settings, elementType: e.target.value as ElementType })}
                    className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white"
                  >
                    <option value="quadrilateral">Quadrilateral (QUAD4/8)</option>
                ...
                <div>
                  <label className="block text-sm text-slate-300 mb-2">Meshing Method</label>
                  <select
                    value={settings.method}
                    onChange={(e) => setSettings({ ...settings, method: e.target.value as MeshMethod })}
                    className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white"
                  >
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-sm text-slate-300 mb-2">Min Size (mm)</label>
                    <input
                      type="number"
                      value={settings.minSize}
                      onChange={(e) => setSettings({ ...settings, minSize: parseInt(e.target.value) })}
                      className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white"
                    />
                  </div>
                  <div>
                    <label className="block text-sm text-slate-300 mb-2">Max Size (mm)</label>
                    <input
                      type="number"
              ...
              {showAdvanced && (
                <div className="mt-4 space-y-4 pt-4 border-t border-slate-700">
                  <div>
                    <label className="block text-sm text-slate-300 mb-2">Refinement Type</label>
                    <select
                      value={settings.refinement}
                      onChange={(e) => setSettings({ ...settings, refinement: e.target.value as RefinementType })}
                      className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white"
                    >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setSettings({ ...settings, curvatureAngle: parseInt(e.target.value) })}
                      className="w-full h-2 bg-slate-700 rounded-lg appearance-none cursor-pointer"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setSettings({ ...settings, smoothingIterations: parseInt(e.target.value) })}
                      className="w-full h-2 bg-slate-700 rounded-lg appearance-none cursor-pointer"
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-sm text-slate-300 mb-2">
                      Quality Target: {(settings.qualityTarget * 100).toFixed(0)}%
                    </label>
                    <input
                      type="range"
                      min="0.5"
                      ...
                      onChange={(e) => setSettings({ ...settings, qualityTarget: parseFloat(e.target.value) })}
                      className="w-full h-2 bg-slate-700 rounded-lg appearance-none cursor-pointer"
                    />
                  </div>
                </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50">
                ...
                <p className="text-2xl font-bold text-white">{metrics.totalElements.toLocaleString()}</p>
              </div>
              ...
              <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50">
                ...
                <p className="text-2xl font-bold text-white">{metrics.totalNodes.toLocaleString()}</p>
              </div>
              ...
              <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50">
                ...
                <p className={`text-2xl font-bold ${getQualityColor(metrics.qualityScore)}`}>
              ...
              <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50">
                ...
                <p className={`text-2xl font-bold ${getQualityColor(metrics.passRate)}`}>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-slate-700">
                      <th className="text-left p-3 text-slate-400 text-sm">Region</th>
                      <th className="text-center p-3 text-slate-400 text-sm">Type</th>
                      <th className="text-center p-3 text-slate-400 text-sm">Element</th>
                      <th className="text-center p-3 text-slate-400 text-sm">Size</th>
                      <th className="text-right p-3 text-slate-400 text-sm">Elements</th>
                      <th className="text-center p-3 text-slate-400 text-sm">Quality</th>
                    ...
                    {regions.map(region => (
                      <tr 
                        ...
                        className={`border-b border-slate-700/50 cursor-pointer transition-colors ${
                          selectedRegion === region.id ? 'bg-cyan-900/20' : 'hover:bg-slate-700/30'
                        }`}
                      >
                        <td className="p-3 text-white font-medium">{region.name}</td>
                        <td className="p-3 text-center text-slate-400 capitalize">{region.type}</td>
                        <td className="p-3 text-center text-slate-400 capitalize">{region.elementType}</td>
                        <td className="p-3 text-center text-slate-400">{region.localSize} mm</td>
                        <td className="p-3 text-right text-slate-400">{region.elements.toLocaleString()}</td>
                        <td className="p-3 text-center">
                          <span className={`font-medium ${getQualityColor(region.quality)}`}>
                            {region.quality}%
                          </span>
                        </td>
                        <td className="p-3 text-center">{getStatusBadge(region.status)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              {showQualityDetails && (
                <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="p-4 bg-slate-700/30 rounded-lg">
                    <p className="text-slate-400 text-xs mb-1">Avg Aspect Ratio</p>
                  ...
                  <div className="p-4 bg-slate-700/30 rounded-lg">
                    <p className="text-slate-400 text-xs mb-1">Max Aspect Ratio</p>
