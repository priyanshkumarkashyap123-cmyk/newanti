...
const CloudStorageDashboard: React.FC = () => {
  ...
  const [versions] = useState<ProjectVersion[]>([
    { id: 'v23', version: 23, timestamp: new Date('2025-01-29T14:30:00'), author: 'You', description: 'Updated seismic analysis results', size: 45670000, isAutoSave: false },
    { id: 'v22', version: 22, timestamp: new Date('2025-01-29T12:15:00'), author: 'You', description: 'Auto-save', size: 45650000, isAutoSave: true },
    { id: 'v21', version: 21, timestamp: new Date('2025-01-29T10:00:00'), author: 'Sarah', description: 'Added pushover analysis', size: 45600000, isAutoSave: false },
    { id: 'v20', version: 20, timestamp: new Date('2025-01-28T16:45:00'), author: 'You', description: 'Revised column sections', size: 45400000, isAutoSave: false },
    { id: 'v19', version: 19, timestamp: new Date('2025-01-28T14:30:00'), author: 'John', description: 'Foundation design update', size: 45300000, isAutoSave: false },
  ]);
  ...
  const getProjectIcon = (type: string) => {
    switch (type) {
      ...
      case 'bridge': return Box;
      case 'industrial': return Folder;
      case 'foundation': return HardDrive;
      default: return File;
    }
  };
  ...
  const getSyncStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'syncing': return 'text-blue-400 animate-spin';
      ...
      <div className="max-w-7xl mx-auto px-6 py-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50">
            ...
            <p className="text-2xl font-bold text-white">{formatSize(storageStats.used)}</p>
          ...
          <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50">
            ...
            <p className="text-lg font-bold text-white">{formatDate(storageStats.lastSync)}</p>
          ...
          <div className="bg-blue-900/30 border border-blue-500/30 rounded-xl p-4 mb-6">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-white font-medium">Uploading project...</span>
              </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-slate-800/50 border border-slate-700 rounded-lg text-white placeholder-slate-400 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none"
            />
          </div>
          ...
          <div className="flex items-center gap-3">
            <select
              ...
              className="px-4 py-2.5 bg-slate-800/50 border border-slate-700 rounded-lg text-white focus:border-blue-500 outline-none"
            >
            ...
            <div className="flex items-center border border-slate-700 rounded-lg overflow-hidden">
              <button
                ...
                className={`p-2.5 ${viewMode === 'grid' ? 'bg-blue-600 text-white' : 'bg-slate-800/50 text-slate-400 hover:text-white'}`}
              >
                <Grid className="w-4 h-4" />
              </button>
              <button
                onClick={() => setViewMode('list')}
                className={`p-2.5 ${viewMode === 'list' ? 'bg-blue-600 text-white' : 'bg-slate-800/50 text-slate-400 hover:text-white'}`}
              >
            ...
            {filteredProjects.map(project => {
              ...
              return (
                <div
                  ...
                  className={`bg-slate-800/50 rounded-xl p-5 border transition-all cursor-pointer group ${
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      <RefreshCw className={`w-4 h-4 ${getSyncStatusColor(project.syncStatus)}`} />
                      <button 
          ...
          <div className="bg-slate-800/50 rounded-xl border border-slate-700/50 overflow-hidden">
            <table className="w-full">
              <thead className="bg-slate-900/50">
                <tr>
                  <th className="text-left p-4 text-slate-400 font-medium text-sm">Project</th>
                  <th className="text-left p-4 text-slate-400 font-medium text-sm">Type</th>
                  <th className="text-left p-4 text-slate-400 font-medium text-sm">Modified</th>
                  <th className="text-right p-4 text-slate-400 font-medium text-sm">Size</th>
                  <th className="text-center p-4 text-slate-400 font-medium text-sm">Version</th>
                  ...
                  return (
                    <tr 
                      ...
                      className={`border-t border-slate-700/50 cursor-pointer transition-colors ${
                        selectedProject?.id === project.id ? 'bg-blue-900/20' : 'hover:bg-slate-700/30'
                      }`}
                    >
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                      ...
                      <td className="p-4 text-slate-400 capitalize">{project.type}</td>
                      <td className="p-4 text-slate-400">{formatDate(project.lastModified)}</td>
                      <td className="p-4 text-right text-slate-400">{formatSize(project.size)}</td>
                      <td className="p-4 text-center text-slate-400">v{project.version}</td>
                      <td className="p-4 text-center">
                        <RefreshCw className={`w-4 h-4 mx-auto ${getSyncStatusColor(project.syncStatus)}`} />
                      </td>
                      <td className="p-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button className="p-1.5 hover:bg-slate-600/50 rounded text-slate-400 hover:text-white">
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      ...
                      {selectedProject.collaborators.map(email => (
                        <div key={email} className="flex items-center gap-2 p-2 bg-slate-800/50 rounded-lg">
        ...
        <div className="mt-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {[
              { name: 'Multi-Story Building', type: 'RCC Frame', icon: Building2, color: 'from-blue-600 to-cyan-600' },
              { name: 'Bridge Structure', type: 'PSC/Steel', icon: Box, color: 'from-green-600 to-emerald-600' },
              { name: 'Industrial Shed', type: 'Steel Portal', icon: Folder, color: 'from-orange-600 to-red-600' },
              { name: 'Foundation', type: 'Isolated/Combined', icon: HardDrive, color: 'from-purple-600 to-pink-600' },
            ].map(template => (
              <button
                key={template.name}
                className="p-4 bg-slate-800/50 rounded-xl border border-slate-700/50 hover:border-slate-600 transition-all group text-left"
              >
