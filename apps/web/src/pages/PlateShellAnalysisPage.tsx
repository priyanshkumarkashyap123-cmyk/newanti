      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-4">
            {/* Panel selector */}
            <div className="flex flex-wrap gap-1 bg-[#0b1326] rounded-xl p-1 border border-[#1a2333]">
            ...
            {activePanel === 'geometry' && (
              <div className="bg-[#0b1326] rounded-xl p-5 border border-[#1a2333] space-y-4">
                ...
                <div className="pt-3 border-t border-[#1a2333] space-y-3">
                  ...
                  <div className="flex flex-col gap-2">
                    {([
                      ...
                      <label key={opt.value} className="flex items-start gap-3 cursor-pointer group">
                        ...
                        />
                        <div>
                          <span className="text-sm text-[#dae2fd] group-hover:text-violet-300 transition-colors">{opt.label}</span>
            ...
            {activePanel === 'mesh' && (
              <div className="bg-[#0b1326] rounded-xl p-5 border border-[#1a2333] space-y-4">
                ...
                <div className="pt-3 border-t border-[#1a2333] grid grid-cols-3 gap-3 text-xs text-center">
                  <div className="bg-[#131b2e] p-2 rounded-lg">
                    <span className="text-slate-500">Nodes</span>
                    <p className="text-[#dae2fd] font-mono mt-1">{totalNodes}</p>
                  </div>
                  <div className="bg-[#131b2e] p-2 rounded-lg">
                    <span className="text-slate-500">Elements</span>
                    <p className="text-[#dae2fd] font-mono mt-1">{totalElements}</p>
                  </div>
                  <div className="bg-[#131b2e] p-2 rounded-lg">
                    <span className="text-slate-500">DOF</span>
                    <p className="text-[#dae2fd] font-mono mt-1">{totalDOF}</p>
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="bg-[#0b1326] rounded-xl p-5 border border-[#1a2333] space-y-4">
                ...
                {loads.map((load, idx) => (
                  <div key={idx} className="bg-[#131b2e] rounded-lg p-4 space-y-3">
                    ...
                    <div className="flex flex-col gap-1">
                      <label className="text-xs text-[#869ab8]">Type</label>
                      <select
                        value={load.type}
                        onChange={e => {
                          const updated = [...loads];
                        ...
                        className="w-full bg-[#0b1326] border border-slate-600 rounded-lg px-3 py-2 text-sm text-[#dae2fd] outline-none"
                      >
                ...
                <button type="button"
                  ...
                  className="w-full py-2 border border-dashed border-slate-600 rounded-lg text-[#869ab8] hover:text-slate-900 dark:hover:text-white hover:border-slate-500 transition-colors text-sm"
                >
                  + Add Load Case
                </button>
              </div>
                ...
                {([
                  ...
                  <div key={edge.key} className="flex flex-col gap-1">
                    ...
                    <select
                      ...
                      className="w-full bg-[#131b2e] border border-slate-600 rounded-lg px-3 py-2 text-sm text-[#dae2fd] outline-none"
                    >
                      {edgeOptions.map(o => (
                ...
                <div className="pt-3 border-t border-[#1a2333]">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button type="button"
                      ...
                      className="px-3 py-2 bg-[#131b2e] border border-slate-600 rounded-lg text-xs text-[#adc6ff] hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
                    >SSSS</button>
                    <button type="button"
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {/* Visualization area */}
            <div className="bg-[#0b1326] rounded-xl border border-[#1a2333] overflow-hidden" style={{ minHeight: 480 }}>
              {!results ? (
                <div className="flex flex-col items-center justify-center h-[480px] text-center px-6">
                  ...
                  <h3 className="text-lg text-[#869ab8] mb-2">Configure & Run Analysis</h3>
                ...
                <div className="flex flex-col items-center justify-center h-[480px] text-center px-6">
                  ...
                  <p className="text-xs text-[#869ab8]">{results.message}</p>
                </div>
              ) : (
                <div className="p-4">
            ...
            {results && results.status === 'success' && (
              <>
                ...
                <div className="bg-[#0b1326] rounded-xl border border-[#1a2333] overflow-hidden">
                  <div className="px-5 py-3 border-b border-[#1a2333] flex items-center justify-between">
                    <h3 className="text-sm font-semibold text-[#dae2fd] flex items-center gap-2">
                    ...
                    <button type="button"
                      ...
                      className="flex items-center gap-1.5 text-xs text-[#869ab8] hover:text-slate-900 dark:hover:text-white transition-colors"
                    >
                        ...
                        {[...results.nodes]
                          ...
                          .map(n => {
                            const Ldelta = Math.abs(n.w) > 1e-10
                              ...
                              : Infinity;
                            return (
                              <tr key={n.id} className="border-b border-[#1a2333] hover:bg-[#131b2e]">
                                <td className="px-4 py-2 text-violet-400 font-mono">{n.id}</td>
                                <td className="px-4 py-2 text-right text-[#dae2fd] font-mono">{n.x.toFixed(1)}</td>
                                <td className="px-4 py-2 text-right text-[#dae2fd] font-mono">{n.y.toFixed(1)}</td>
                                <td className="px-4 py-2 text-right text-cyan-400 font-mono">{n.w.toFixed(4)}</td>
                                <td className="px-4 py-2 text-right text-[#adc6ff]">{Ldelta < 100000 ? `L/${Ldelta}` : '-'}</td>
                              </tr>
                            );
                          })}
                      </tbody>
...
const DisplacementHeatmap: React.FC<{
  ...
  return (
    <div className="flex justify-center">
      <div
        className="grid border border-[#1a2333] rounded"
        style={{
      ...
      >
        {results.nodes.map(node => (
          <div
            ...
            className="border border-[#1a2333]/30 transition-colors"
            style={{
