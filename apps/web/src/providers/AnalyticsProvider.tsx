...
import { createContext, useContext, useCallback, FC, ReactNode, useEffect } from 'react';
...
export const AnalyticsProvider: FC<AnalyticsProviderProps> = ({ children, debug = false }) => {
  ...
  const sendEvent = useCallback(async (event: AnalyticsEvent) => {
    ...
    // Examples: Mixpanel, Amplitude, PostHog, or custom backend
    try {
 ...
 */
export const useTrackMount = (eventName: string, properties?: Record<string, any>) => {
 ...
 */
export const useFunnelStep = (funnelName: string, stepName: string, stepNumber: number) => {
  const { track } = useAnalytics();
