...
export const WorkspaceLayout: FC<WorkspaceLayoutProps> = ({ children }) => {
    ...
    // Prepare nodes/members for data tables
    const nodeData = Object.entries(nodes).map(([id, node]) => ({
        id,
        x: node.x,
        y: node.y,
        z: node.z
    ...
    const memberData = Object.entries(members).map(([id, member]) => ({
        id,
            ...
            <header className="h-10 flex items-center justify-between px-4 bg-zinc-100 dark:bg-zinc-950 border-b border-zinc-200 dark:border-zinc-800">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        ...
                        className="p-1.5 rounded text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 transition-colors"
                    >
                        {isSidebarCollapsed ? (
