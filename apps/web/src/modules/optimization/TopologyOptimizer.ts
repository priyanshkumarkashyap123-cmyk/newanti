...
export interface OptimizationResult {
    ...
    convergenceHistory: { iteration: number; compliance: number; change: number }[];
    converged: boolean;
...
export class TopologyOptimizer {
    ...
    optimize(
        ...
        const { volumeFraction, penalization, maxIterations, tolerance, minDensity, moveLimit } = this.params;
        ...
        for (let iter = 0; iter < maxIterations; iter++) {
            // Assemble global stiffness matrix and solve
            const { U, compliance } = this.solveSystem(densities, ke, supports, loads);
            ...
            const { newDensities, change } = this.updateDensities(
                densities, dcFiltered, volumeFraction, moveLimit, minDensity
            );
        ...
        auditTrail.log('optimization', 'topology_optimization',
     ...
     */
    private prepareFilter(): Map<string, { weight: number; neighbors: { i: number; j: number }[] }> {
        const { nelx, nely } = this.domain;
        const rmin = this.params.filterRadius;
        const H = new Map<string, { weight: number; neighbors: { i: number; j: number }[] }>();
    ...
    private solveSystem(
        ...
        for (let j = 0; j < nely; j++) {
            for (let i = 0; i < nelx; i++) {
                ...
                const n3 = n2 + (nelx + 1);
                const n4 = n1 + (nelx + 1);
                const edof = [2 * n1, 2 * n1 + 1, 2 * n2, 2 * n2 + 1,
        ...
        for (const bc of supports) {
            for (const nodeIdx of bc.nodeIndices) {
                if (bc.direction === 'x' || bc.direction === 'xy' || bc.type === 'fixed') {
                    fixedDofs.add(2 * nodeIdx);
                }
                if (bc.direction === 'y' || bc.direction === 'xy' || bc.type === 'fixed') {
    ...
    private updateDensities(
        ...
        while (l2 - l1 > 1e-9) {
            ...
            for (let j = 0; j < nely; j++) {
                ...
                for (let i = 0; i < nelx; i++) {
                    ...
                    // Apply move limits
                    xnew = Math.max(rhoMin, Math.max(xold - move, Math.min(1, Math.min(xold + move, xnew))));
        ...
        for (let j = 0; j < nely; j++) {
            for (let i = 0; i < nelx; i++) {
                change = Math.max(change, Math.abs(newDensities[j][i] - densities[j][i]));
            }
        }
     ...
     */
    private interpretResult(result: OptimizationResult, targetVol: number): string[] {
        const interpretations: string[] = [];

        if (result.converged) {
            interpretations.push(`✅ Optimization converged in ${result.iterations} iterations`);
        } else {
            interpretations.push(`⚠️ Did not fully converge (${result.iterations} iterations)`);
        }

        interpretations.push(`Achieved volume: ${(result.volume * 100).toFixed(1)}% (target: ${(targetVol * 100).toFixed(0)}%)`);
        interpretations.push(`Compliance (flexibility): ${result.compliance.toExponential(3)}`);
        interpretations.push(`Computation time: ${result.computationTime.toFixed(0)}ms`);

        // Analyze density distribution
        const { nelx, nely } = this.domain;
        let solidCount = 0, voidCount = 0, intermediateCount = 0;
        ...
        interpretations.push(`Structure: ${((solidCount / total) * 100).toFixed(0)}% solid, ${((voidCount / total) * 100).toFixed(0)}% void`);

        if (intermediateCount > total * 0.1) {
            interpretations.push(`⚠️ High intermediate density (${((intermediateCount / total) * 100).toFixed(0)}%) - consider post-processing`);
        }
...
): { optimizer: TopologyOptimizer; supports: BoundaryCondition[]; loads: LoadCondition[] } {
