/**
 * Design Routes - Structural Member Design API
 * 
...
interface SteelDesignRequest {
    ...
    designMethod?: 'LRFD' | 'ASD';  // For AISC 360
}

interface ConcreteBeamDesignRequest {
    section: {
        width: number;           // mm
        depth: number;           // mm
...
interface ConcreteColumnDesignRequest {
    section: {
        width: number;   // mm
        depth: number;   // mm
        cover: number;   // mm
    ...
    forces: {
        ...
        Mux: number;     // Moment X (kNm)
        Muy: number;     // Moment Y (kNm)
    };
    geometry: {
        unsupportedLength: number;       // mm
...
interface ConnectionDesignRequest {
    ...
    plate?: {
        thickness: number;   // mm
        fy: number;          // MPa
        width?: number;      // mm (for base plate)
        length?: number;     // mm (for base plate)
    };
    material?: {
        fu: number;          // Ultimate strength (MPa)
        fy: number;          // Yield strength (MPa)
...
interface FootingDesignRequest {
    type: 'isolated' | 'combined' | 'mat';
    loads: Array<{
        P: number;           // Axial load (kN)
        Mx?: number;         // Moment X (kNm)
        My?: number;         // Moment Y (kNm)
        x?: number;          // X coordinate (m) - for combined/mat
        y?: number;          // Y coordinate (m) - for combined/mat
    }>;
    columnSize: {
        width: number;       // mm
        depth: number;       // mm
...
// ============================================
// HELPER: Run Python Design Script
// ============================================

async function runPythonDesign(
    scriptPath: string,
    inputData: any
): Promise<any> {
