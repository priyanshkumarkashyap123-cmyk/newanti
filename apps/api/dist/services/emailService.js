...
const EMAIL_SERVICE = process.env["EMAIL_SERVICE"] || "nodemailer";
...
const emailTemplates = {
                ...
                <body>
                    <div class="container">
                        <div class="email-box">
                            ...
                            <div class="footer">
                                <p>\xA9 2025 BeamLab. All rights reserved.</p>
  ...
  resetPassword: (name, resetToken, email) => {
  ...
  emailChangeConfirmation: (name, code) => {
    return {
...
const emailService = {
  ...
  sendVerificationEmail: async (email, name, code) => {
    try {
      if (!transporter) {
        console.log(`\u{1F4E7} [DEV MODE] Verification code for ${email}: ${code}`);
        return true;
      }
  ...
  sendPasswordResetEmail: async (email, name, resetToken) => {
    try {
      if (!transporter) {
        console.log(`\u{1F4E7} [DEV MODE] Password reset link for ${email}: /reset-password?token=${resetToken}`);
        return true;
      }
  ...
  sendWelcomeEmail: async (email, name) => {
    try {
      if (!transporter) {
        console.log(`\u{1F4E7} [DEV MODE] Welcome email for ${email}`);
        return true;
      }
   ...
   */
  sendEmailChangeConfirmation: async (email, name, code) => {
    try {
      if (!transporter) {
        console.log(`\u{1F4E7} [DEV MODE] Email change confirmation for ${email}: ${code}`);
        return true;
      }
...
export {
  emailService_default as default,
  emailService
};
