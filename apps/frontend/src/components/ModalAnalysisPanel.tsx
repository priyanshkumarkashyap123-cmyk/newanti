...
const CumulativeMassChart: FC<{
    ...
    return (
        <div className="h-32 relative bg-[#131b2e] rounded p-2">
...
export const ModalAnalysisPanel: FC<ModalAnalysisPanelProps> = ({ isPro = false }) => {
    ...
    if (!isPro) {
        return (
            <div className="p-4 bg-gradient-to-br from-purple-50 to-indigo-50 dark:from-purple-900/20 dark:to-indigo-900/20 rounded-lg border border-[#1a2333]">
    ...
    return (
        <div className="flex flex-col h-full bg-[#0b1326] text-[#dae2fd] rounded-lg overflow-hidden">
            {/* Header */}
            <div className="p-3 border-b border-[#1a2333] bg-[#0b1326]">
                <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex gap-2 text-xs">
                        <button type="button"
                            ...
                            className={`px-2 py-1 rounded ${viewMode === 'cards' ? 'bg-slate-200 dark:bg-slate-700 text-[#dae2fd]' : 'text-[#869ab8] hover:text-slate-900 dark:hover:text-white'}`}
                        >
                            Cards
                        </button>
                        <button type="button"
                            onClick={() => setViewMode('table')}
                            className={`px-2 py-1 rounded ${viewMode === 'table' ? 'bg-slate-200 dark:bg-slate-700 text-[#dae2fd]' : 'text-[#869ab8] hover:text-slate-900 dark:hover:text-white'}`}
                        >
                            Table
                        </button>
                    </div>
                ...
                <div className="flex items-center gap-3 text-xs">
                    <label className="flex items-center gap-2">
                        ...
                        <input
                            ...
                            className="w-16 px-2 py-1 border border-[#1a2333] rounded bg-[#131b2e] text-[#dae2fd] focus:outline-none focus:border-blue-500"
                        />
                    </label>
                    <button type="button"
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-3 gap-2 mb-4 text-center text-xs">
                            <div className="p-2 bg-[#131b2e] rounded border border-[#1a2333]">
                                ...
                                <div className="text-[#869ab8]">1st Freq (Hz)</div>
                            </div>
                            <div className="p-2 bg-[#131b2e] rounded border border-[#1a2333]">
                                <div className="text-lg font-bold text-green-400">{modes[0]?.period.toFixed(3)}</div>
                                <div className="text-[#869ab8]">1st Period (s)</div>
                            </div>
                            <div className="p-2 bg-[#131b2e] rounded border border-[#1a2333]">
                                <div className="text-lg font-bold text-slate-700 dark:text-slate-200">{modes.length}</div>
                                <div className="text-[#869ab8]">Modes</div>
                            </div>
                        </div>

                        {viewMode === 'table' ? (
                            <div className="bg-[#131b2e] rounded-lg overflow-hidden text-xs border border-[#1a2333]">
                                <table className="w-full text-left">
                                    ...
                                    <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                                        {modes.map((mode, index) => (
                                            <tr
                                                key={mode.modeNumber}
                                                className={`hover:bg-slate-200/50 dark:hover:bg-slate-700/50 cursor-pointer ${activeModeIndex === index ? 'bg-blue-900/20' : ''}`}
                                            ...
                                            >
                                                ...
                                                <td className="p-2 text-center">
                                                    <button type="button"
                                                        ...
                                                        className="text-[#869ab8] hover:text-blue-400"
                                                    >
            ...
            {/* Footer */}
            {modes.length > 0 && (
                <div className="p-3 border-t border-[#1a2333] bg-[#0b1326] flex justify-between items-center text-xs">
