                    ...
                    <div className="py-4">
                        {activeTab === 'releases' && (
                            <div className="space-y-6">
                                <div className="grid grid-cols-2 gap-8">
                                    {/* Start Node */}
                                    <div>
                                        <h3 className="text-sm font-semibold text-[#adc6ff] mb-3 border-b border-[#1a2333] pb-1">Start Node</h3>
                                        <div className="space-y-2">
                                            {['fx', 'fy', 'fz', 'mx', 'my', 'mz'].map((dof) => (
                                                <label key={`start-${dof}`} className="flex items-center justify-between group cursor-pointer">
                                                    <span className="text-sm text-[#869ab8] group-hover:text-slate-700 dark:group-hover:text-slate-300 font-mono uppercase">{dof}</span>
                                                    <input
                                                        type="checkbox"
                                                        checked={(releases as any)[`${dof}Start`]}
                                                        onChange={(e) => setReleases(prev => ({ ...prev, [`${dof}Start`]: e.target.checked }))}
                                                        className="w-4 h-4 rounded border-slate-300 dark:border-slate-600 bg-[#131b2e] text-violet-600 focus:ring-violet-500 focus:ring-offset-white dark:focus:ring-offset-slate-900"
                                                    />
                                                </label>
                                            ))}
                                        </div>
                                    ...
                                    {/* End Node */}
                                    <div>
                                        <h3 className="text-sm font-semibold text-[#adc6ff] mb-3 border-b border-[#1a2333] pb-1">End Node</h3>
                                        <div className="space-y-2">
                                            {['fx', 'fy', 'fz', 'mx', 'my', 'mz'].map((dof) => (
                                ...
                                <p className="text-xs text-[#869ab8] italic">
                        ...
                        {activeTab === 'offsets' && (
                            <div className="space-y-6">
                                {/* Start Node Offsets */}
                                <div>
                                    <h3 className="text-sm font-semibold text-[#adc6ff] mb-3">Start Node Offsets (Global)</h3>
                                ...
                                {/* End Node Offsets */}
                                <div>
                                    <h3 className="text-sm font-semibold text-[#adc6ff] mb-3">End Node Offsets (Global)</h3>
                                    <div className="grid grid-cols-3 gap-3">
                                        {['x', 'y', 'z'].map((axis) => (
                                            <div key={`end-${axis}`}>
                                                <Label className="text-xs uppercase mb-1">{axis} (m)</Label>
