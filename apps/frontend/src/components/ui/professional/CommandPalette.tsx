...
const CATEGORY_CONFIG: Record<CommandCategory, { icon: React.ElementType; color: string; label: string }> = {
  ...
  settings: { icon: Settings, color: 'text-[#869ab8]', label: 'Settings' },
  help: { icon: Command, color: 'text-indigo-400', label: 'Help' },
  recent: { icon: Clock, color: 'text-[#869ab8]', label: 'Recent' },
};
...
const SearchInput: FC<{
  ...
  return (
    <div className="flex items-center gap-2 px-4 py-3 border-b border-[#1a2333]">
      {getModeIcon()}
      <input
        ref={inputRef}
        type="text"
        ...
        autoFocus
      />
      <div className="flex items-center gap-1 text-[10px] text-[#869ab8]">
        <kbd className="px-1.5 py-0.5 bg-[#131b2e] rounded">↑↓</kbd>
        <span>navigate</span>
        <kbd className="px-1.5 py-0.5 bg-[#131b2e] rounded ml-2">Enter</kbd>
        <span>select</span>
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {command.description && (
          <div className="text-xs text-[#869ab8] truncate">{command.description}</div>
        )}
      </div>
      <div className="flex items-center gap-2">
        <span className={`text-[10px] px-1.5 py-0.5 rounded ${categoryConfig.color} bg-[#131b2e]`}>
          {categoryConfig.label}
        </span>
        {command.shortcut && (
          <kbd className="px-1.5 py-0.5 text-[10px] bg-slate-200 dark:bg-slate-700 text-[#adc6ff] rounded">
            {command.shortcut}
          </kbd>
        )}
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs text-[#869ab8] truncate">{file.path}</div>
      </div>
      {file.modified && (
...
const SymbolItem: FC<{
  ...
  return (
    <div
      ...
      onMouseEnter={onMouseEnter}
    >
      <Icon className={`w-4 h-4 ${typeColors[symbol.type] || 'text-[#869ab8]'}`} />
      <div className="flex-1 min-w-0">
        <div className="text-sm text-slate-800 dark:text-slate-200 truncate">{symbol.name}</div>
        {symbol.location && (
          <div className="text-xs text-[#869ab8] truncate">{symbol.location}</div>
        )}
      </div>
      <span className={`text-[10px] px-1.5 py-0.5 rounded bg-[#131b2e] ${typeColors[symbol.type] || 'text-[#869ab8]'}`}>
        {symbol.type}
      </span>
    </div>
  );
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-[600px] max-h-[60vh] p-0 gap-0 top-[15vh] translate-y-0 overflow-hidden">
          ...
          {/* Mode Tabs */}
          <div className="flex items-center gap-2 px-4 py-2 border-b border-[#1a2333] bg-[#0b1326]">
            {[
              ...
              <button type="button"
                ...
                className={`
                  ...
                  ${mode === tab.mode 
                    ...
                    : 'text-[#869ab8] hover:text-slate-700 dark:hover:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-800'
                  }
                `}
              >
          ...
          <div ref={listRef} className="overflow-y-auto max-h-[calc(60vh-120px)]">
            {mode === 'goto' ? (
              <div className="px-4 py-8 text-center text-[#869ab8]">
                <Hash className="w-8 h-8 mx-auto mb-2 opacity-50" />
                <p className="text-sm">Type a line number to navigate</p>
                {query && !isNaN(parseInt(query)) && (
                  <p className="mt-2 text-[#869ab8]">
                    Press <kbd className="px-1.5 py-0.5 bg-[#131b2e] rounded text-xs">Enter</kbd> to go to line {query}
                  </p>
                )}
              </div>
              ...
              query ? (
                ...
                Object.entries(groupedCommands || {}).map(([category, cmds]) => {
                  ...
                  return (
                    <div key={category}>
                      <div className="flex items-center gap-2 px-4 py-1.5 bg-[#131b2e] sticky top-0">
                        <CategoryIcon className={`w-3.5 h-3.5 ${config.color}`} />
                        <span className="text-[10px] font-semibold text-[#869ab8] uppercase tracking-wider">
                          {config.label}
                        </span>
                      </div>
                      {cmds.map((cmd) => {
          ...
          {/* Footer */}
          <div className="px-4 py-2 border-t border-[#1a2333] bg-[#0b1326] flex items-center justify-between text-[10px] text-[#869ab8]">
            <div className="flex items-center gap-4">
              <span>
                <kbd className="px-1 py-0.5 bg-[#131b2e] rounded">Ctrl</kbd>+
                <kbd className="px-1 py-0.5 bg-[#131b2e] rounded">K</kbd> Quick search
              </span>
              <span>
                <kbd className="px-1 py-0.5 bg-[#131b2e] rounded">Ctrl</kbd>+
                <kbd className="px-1 py-0.5 bg-[#131b2e] rounded">P</kbd> Command palette
              </span>
            </div>
