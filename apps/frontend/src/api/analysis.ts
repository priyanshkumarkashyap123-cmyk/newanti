...
async function runAnalysis(): Promise<{
  ...
  try {
    ...
    // Extract numeric portion from prefixed IDs (e.g., "N1" → 1, "M3" → 3)
    const extractId = (id: string): number => parseInt(id.replace(/\D/g, '')) || 0;
    ...
    const wasmElements = members.map((m) => ({
      id: extractId(m.id),
      node_start: extractId(m.startNodeId),
      node_end: extractId(m.endNodeId),
      e: m.E || 200e9,
      i: m.I || 8.33e-6,
      a: m.A || 0.01,
    }));
    ...
    const wasmMemberLoads: WasmMemberLoad[] = memberLoads
      ...
      .map((ml) => ({
        element_id: extractId(ml.memberId),
        w1: ml.w1 ?? 0,
