...
class MatrixOps {
  ...
  static determinant3x3(A: number[][]): number {
    ...
    return (
      (A[0][0] || 0) *
        ((A[1][1] || 0) * (A[2][2] || 0) - (A[1][2] || 0) * (A[2][1] || 0)) -
      (A[0][1] || 0) *
        ((A[1][0] || 0) * (A[2][2] || 0) - (A[1][2] || 0) * (A[2][0] || 0)) +
      (A[0][2] || 0) *
        ((A[1][0] || 0) * (A[2][1] || 0) - (A[1][1] || 0) * (A[2][0] || 0))
    );
  }

  static inverse2x2(A: number[][]): number[][] {
    const det = this.determinant2x2(A);
    if (Math.abs(det) < 1e-12 || !A[0] || !A[1]) {
      throw new Error("Matrix is singular");
    }
    return [
      [(A[1][1] || 0) / det, -(A[0][1] || 0) / det],
      [-(A[1][0] || 0) / det, (A[0][0] || 0) / det],
    ];
  }

  static inverse3x3(A: number[][]): number[][] {
    const det = this.determinant3x3(A);
    if (Math.abs(det) < 1e-12 || !A[0] || !A[1] || !A[2]) {
      throw new Error("Matrix is singular");
    }
    ...
    if (inv[0] && inv[1] && inv[2]) {
      inv[0][0] =
        ...
        det;
      inv[0][1] =
        ((A[0][2] || 0) * (A[2][1] || 0) - (A[0][1] || 0) * (A[2][2] || 0)) /
        det;
      inv[0][2] =
        ((A[0][1] || 0) * (A[1][2] || 0) - (A[0][2] || 0) * (A[1][1] || 0)) /
        det;
      inv[1][0] =
        ((A[1][2] || 0) * (A[2][0] || 0) - (A[1][0] || 0) * (A[2][2] || 0)) /
        det;
      inv[1][1] =
        ((A[0][0] || 0) * (A[2][2] || 0) - (A[0][2] || 0) * (A[2][0] || 0)) /
        det;
      inv[1][2] =
        ((A[0][2] || 0) * (A[1][0] || 0) - (A[0][0] || 0) * (A[1][2] || 0)) /
        det;
      inv[2][0] =
        ((A[1][0] || 0) * (A[2][1] || 0) - (A[1][1] || 0) * (A[2][0] || 0)) /
        det;
      inv[2][1] =
        ((A[0][1] || 0) * (A[2][0] || 0) - (A[0][0] || 0) * (A[2][1] || 0)) /
        det;
      inv[2][2] =
  ...
  static solveLU(A: number[][], b: number[]): number[] {
    ...
    for (let i = 0; i < n; i++) {
      ...
      for (let k = i; k < n; k++) {
        let sum = 0;
        for (let j = 0; j < i; j++) {
          if (L[i] && U[j]) sum += (L[i]![j] || 0) * (U[j]![k] || 0);
        }
        if (U[i] && A[i]) U[i]![k] = (A[i]![k] || 0) - sum;
      }
      ...
      for (let k = i; k < n; k++) {
        if (i === k) {
          ...
          let sum = 0;
          for (let j = 0; j < i; j++) {
            if (L[k] && U[j]) sum += (L[k]![j] || 0) * (U[j]![i] || 0);
          }
          if (L[k] && A[k] && U[i])
            L[k]![i] = ((A[k]![i] || 0) - sum) / (U[i]![i] || 1);
        }
      }
    }
    ...
    for (let i = n - 1; i >= 0; i--) {
      let sum = 0;
      for (let j = i + 1; j < n; j++) {
        if (U[i]) sum += (U[i]![j] || 0) * x[j];
      }
      if (U[i]) x[i] = (y[i] - sum) / (U[i]![i] || 1);
    }
  ...
  static solveCholesky(A: number[][], b: number[]): number[] {
    ...
    for (let i = 0; i < n; i++) {
      for (let j = 0; j <= i; j++) {
        ...
        if (i === j) {
          for (let k = 0; k < j; k++) {
            if (L[j]) sum += (L[j]![k] || 0) * (L[j]![k] || 0);
          }
          if (L[j] && A[j])
            L[j]![j] = Math.sqrt(Math.max(0, (A[j]![j] || 0) - sum));
        } else {
          for (let k = 0; k < j; k++) {
            if (L[i] && L[j]) sum += (L[i]![k] || 0) * (L[j]![k] || 0);
          }
          if (L[i] && A[i] && L[j])
            L[i]![j] = ((A[i]![j] || 0) - sum) / (L[j]![j] || 1);
        }
      }
    }
    ...
    for (let i = n - 1; i >= 0; i--) {
      let sum = 0;
      for (let j = i + 1; j < n; j++) {
        if (L[j]) sum += (L[j]![i] || 0) * x[j];
      }
      if (L[i]) x[i] = (y[i] - sum) / (L[i]![i] || 1);
    }
...
class ElementStiffness {
  ...
  static tri3(
    ...
    if (!n1 || !n2 || !n3) {
      throw new Error("tri3 requires exactly 3 nodes");
    }

    // Area calculation (2A = determinant)
    const area =
      0.5 *
      Math.abs((n2.x - n1.x) * (n3.y - n1.y) - (n3.x - n1.x) * (n2.y - n1.y));
    ...
    for (let i = 0; i < 3; i++) {
      if (B[0] && B[1] && B[2]) {
        B[0][2 * i] = (b[i] || 0) / (2 * area);
        B[1][2 * i + 1] = (c[i] || 0) / (2 * area);
        B[2][2 * i] = (c[i] || 0) / (2 * area);
        B[2][2 * i + 1] = (b[i] || 0) / (2 * area);
      }
    }
  ...
  static quad4(
    ...
    for (const gp of gaussPts) {
      ...
      // Shape functions (unused but kept for reference)
      // const N = [
      //   0.25 * (1 - xi) * (1 - eta),
      //   0.25 * (1 + xi) * (1 - eta),
      //   0.25 * (1 + xi) * (1 + eta),
      ...
      for (let i = 0; i < 4; i++) {
        const node = nodes[i];
        if (node) {
          J11 += (dN_dxi[i] || 0) * node.x;
          J12 += (dN_dxi[i] || 0) * node.y;
          J21 += (dN_deta[i] || 0) * node.x;
          J22 += (dN_deta[i] || 0) * node.y;
        }
      }
      ...
      const dN_dy: number[] = [];
      for (let i = 0; i < 4; i++) {
        dN_dx[i] = (J22 * (dN_dxi[i] || 0) - J12 * (dN_deta[i] || 0)) / detJ;
        dN_dy[i] = (-J21 * (dN_dxi[i] || 0) + J11 * (dN_deta[i] || 0)) / detJ;
      }
      ...
      for (let i = 0; i < 8; i++) {
        for (let j = 0; j < 8; j++) {
          if (K[i] && BTD_B[i]) {
            K[i]![j] = (K[i]![j] || 0) + factor * (BTD_B[i]![j] || 0);
          }
        }
      }
    }
  ...
  static plate4(
    ...
    for (const gp of gaussFull) {
      ...
      for (let i = 0; i < 4; i++) {
        if (K[i * 3] && K[i * 3 + 1] && K[i * 3 + 2]) {
          K[i * 3]![i * 3] = (K[i * 3]![i * 3] || 0) + factor * 0.1;
          K[i * 3 + 1]![i * 3 + 1] =
            (K[i * 3 + 1]![i * 3 + 1] || 0) + factor * 0.2;
          K[i * 3 + 2]![i * 3 + 2] =
            (K[i * 3 + 2]![i * 3 + 2] || 0) + factor * 0.2;
        }
      }
    }
    ...
    for (const gp of gaussReduced) {
      ...
      for (let i = 0; i < 4; i++) {
        if (K[i * 3]) {
          K[i * 3]![i * 3] = (K[i * 3]![i * 3] || 0) + factor * 0.05;
        }
      }
    }
...
export class AdvancedFEAEngine {
  ...
  public applyConstraint(
    nodeId: number,
    constraints: FEANode["constraints"],
  ): void {
  ...
  private staticAnalysis(): FEAResult {
    ...
    for (const element of this.elements) {
      ...
      for (let i = 0; i < Ke.length; i++) {
        for (let j = 0; j < (Ke[i]?.length || 0); j++) {
          const gi = globalDOF[i];
          const gj = globalDOF[j];
          if (gi !== undefined && gj !== undefined && K[gi]) {
            K[gi]![gj] = (K[gi]![gj] || 0) + (Ke[i]![j] || 0);
          }
        }
      }
    }
    ...
    for (const node of this.nodes) {
      if (node.loads) {
        ...
        if (dofPerNode > 2 && node.loads.Fz)
          F[baseDOF + 2] = node.loads.Fz * 1000;
      }
    }
    ...
    for (const node of this.nodes) {
      if (node.constraints) {
        ...
        if (node.constraints.dx && K[baseDOF])
          K[baseDOF]![baseDOF] = (K[baseDOF]![baseDOF] || 0) + penalty;
        if (node.constraints.dy && K[baseDOF + 1])
          K[baseDOF + 1]![baseDOF + 1] =
            (K[baseDOF + 1]![baseDOF + 1] || 0) + penalty;
        if (dofPerNode > 2 && node.constraints.dz && K[baseDOF + 2])
          K[baseDOF + 2]![baseDOF + 2] =
            (K[baseDOF + 2]![baseDOF + 2] || 0) + penalty;
      }
    }
    ...
    const displacements: NodeDisplacement[] = this.nodes.map((node, i) => ({
      nodeId: node.id,
      dx: (U[i * dofPerNode] || 0) * 1000, // mm
      dy: (U[i * dofPerNode + 1] || 0) * 1000,
      dz: dofPerNode > 2 ? (U[i * dofPerNode + 2] || 0) * 1000 : undefined,
    }));
    ...
    const maxDisp = displacements.reduce(
      (max, d) => {
        const mag = Math.sqrt(
          d.dx * d.dx + d.dy * d.dy + (d.dz || 0) * (d.dz || 0),
        );
        return mag > max.value
          ? { nodeId: d.nodeId, value: mag, direction: "Total" }
          : max;
      },
      { nodeId: 0, value: 0, direction: "" },
    );

    const maxStress = stresses.reduce(
      (max, s) =>
        s.vonMises > max.vonMises
          ? { elementId: s.elementId, vonMises: s.vonMises }
          : max,
  ...
  private modalAnalysis(): FEAResult {
    ...
    for (const element of this.elements) {
      ...
      for (let i = 0; i < Ke.length; i++) {
        for (let j = 0; j < (Ke[i]?.length || 0); j++) {
          const gi = globalDOF[i];
          const gj = globalDOF[j];
          if (gi !== undefined && gj !== undefined && K[gi] && M[gi]) {
            K[gi]![gj] = (K[gi]![gj] || 0) + (Ke[i]![j] || 0);
            M[gi]![gj] = (M[gi]![gj] || 0) + (Me[i]![j] || 0);
          }
        }
      }
    }
    ...
    for (let mode = 1; mode <= 5; mode++) {
      ...
      const k00 = K[0]?.[0] || 1;
      const m00 = M[0]?.[0] || 1;
      const f = (mode * 2 * Math.sqrt(Math.max(0, k00 / m00))) / (2 * Math.PI);
  ...
  private getDOFPerNode(): number {
    ...
    if (
      elementTypes.has("TRI3") ||
      elementTypes.has("QUAD4") ||
      elementTypes.has("TRUSS2D")
    )
      return 2;
  ...
  private getElementStiffness(element: FEAElement): number[][] {
    const material =
      this.materials.get(element.materialId) || this.materials.get("default")!;
    const section = element.sectionId
      ? this.sections.get(element.sectionId)
      : undefined;

    const elementNodes = element.nodeIds.map(
      (nid) => this.nodes.find((n) => n.id === nid)!,
    );
    ...
    switch (element.type) {
      case "BEAM2D": {
        ...
        if (!n1 || !n2) return MatrixOps.create(6, 6);
      ...
      case "TRUSS2D": {
        const n1 = elementNodes[0];
        const n2 = elementNodes[1];
        if (!n1 || !n2) return MatrixOps.create(4, 4);
        const dx = n2.x - n1.x;
        const dy = n2.y - n1.y;
    ...
    for (const element of this.elements) {
      ...
      for (const nid of element.nodeIds) {
        ...
        for (let d = 0; d < Math.min(dofPerNode, 2); d++) {
          u.push(U[nodeIndex * dofPerNode + d] || 0);
        }
      }
      ...
      if (element.type === "TRUSS2D" || element.type === "BEAM2D") {
        ...
        if (nodes[0] && nodes[1]) {
          ...
          const strain = ((u[2] || 0) - (u[0] || 0)) / (L || 1);
          sigmaX = E * strain;
        }
      } else {
        // 2D continuum elements
        const avgStrain =
          Math.sqrt(u.reduce((s, v) => s + v * v, 0)) / u.length;
    ...
    for (const node of this.nodes) {
      if (node.constraints?.dx || node.constraints?.dy) {
        ...
        for (let j = 0; j < K.length; j++) {
          if (K[baseDOF]) Rx += (K[baseDOF]![j] || 0) * (U[j] || 0);
          if (K[baseDOF + 1]) Ry += (K[baseDOF + 1]![j] || 0) * (U[j] || 0);
        }
