...
impl TimeHistorySolver {
    ...
    pub fn analyze(
        ...
        let damping = match self.config.damping {
            ...
            DampingModel::Rayleigh { alpha, beta } => alpha * mass + beta * stiffness,
            DampingModel::Modal { .. } => {
        ...
        for step in 1..n_steps {
            ...
            let f_eff =
                f + mass * (a0 * &u + a2 * &v + a3 * &a) + &damping * (a1 * &u + a4 * &v + a5 * &a);
        ...
        for i in 0..n_dof {
            max_displacements[i] = displacements.iter().map(|u| u[i].abs()).fold(0.0, f64::max);

            max_velocities[i] = velocities.iter().map(|v| v[i].abs()).fold(0.0, f64::max);

            max_accelerations[i] = accelerations.iter().map(|a| a[i].abs()).fold(0.0, f64::max);
        }
