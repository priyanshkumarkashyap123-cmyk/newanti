...
// ── Shear Friction Design (IS 456 Cl. 40.6) ──

/// Design for shear at interface (shear friction) per IS 456:2000 Cl. 40.6
///
/// Shear transfer across an interface (e.g., precast-to-cast-in-situ, construction joints)
/// relies on:
/// 1. Clamping action via reinforcement crossing the interface
/// 2. Friction coefficient μ (depends on interface preparation)
/// 3. Concrete cohesion
///
/// **Design equation**:
/// V_u ≤ τ_u × A_v + μ × (A_sf × 0.87 × f_y)
///
/// where:
/// - V_u = factored shear force (N)
/// - τ_u = ultimate shear stress at interface = 0.2 × f_ck (N/mm²) for rough surfaces
/// - A_v = interface contact area (mm²)
/// - μ = coefficient of friction (0.9 rough, 0.7 smooth, 0.6 smooth with bonding agent)
/// - A_sf = area of reinforcement crossing the interface (mm²)
/// - f_y = yield strength of steel crossing interface (N/mm²)
///
/// **Reference**: IS 456:2000 Cl. 40.6
///
/// # Arguments
/// * `vu_kn` - Factored shear force (kN)
/// * `interface_area_mm2` - Contact area at interface (mm²)
/// * `fck_mpa` - Concrete grade (MPa)
/// * `fy_mpa` - Steel yield strength (MPa)
/// * `interface_type` - "rough" (as-cast, intentional roughness), "smooth" (formwork), "smooth_bonding" (bonding agent)
/// * `steel_area_crossing_mm2` - Area of reinforcement crossing the interface (mm²)
///
/// # Returns
/// ShearFrictionResult with capacity, utilization, and pass/fail
pub fn design_shear_friction(
    vu_kn: f64,
    interface_area_mm2: f64,
    fck_mpa: f64,
    fy_mpa: f64,
    interface_type: &str,
    steel_area_crossing_mm2: f64,
) -> ShearFrictionResult {
    // Friction coefficient μ per IS 456 Cl. 40.6
    let mu = match interface_type {
        "rough" => 0.9,          // Roughened to amplitude ≥ 5 mm
        "smooth" => 0.7,         // Smooth cast surface
        "smooth_bonding" => 0.6, // Smooth + bonding agent
        _ => 0.7,                // Default to smooth
    };

    // Ultimate shear stress at interface (IS 456 Cl. 40.6)
    // τ_u = 0.2 × f_ck for rough interfaces
    let tau_u = if interface_type == "rough" {
        0.2 * fck_mpa
    } else {
        0.15 * fck_mpa // More conservative for smooth
    };

    // Concrete contribution (cohesion + dowel action)
    let v_concrete_kn = (tau_u * interface_area_mm2) / 1000.0; // N to kN

    // Steel contribution (friction via clamping)
    // V_steel = μ × A_sf × f_yd where f_yd = 0.87 × f_y (IS 456 Cl. 36.4.2)
    let fyd = 0.87 * fy_mpa;
    let v_steel_kn = (mu * steel_area_crossing_mm2 * fyd) / 1000.0; // N to kN

    // Total capacity
    let v_capacity_kn = v_concrete_kn + v_steel_kn;

    // Check
    let passed = vu_kn <= v_capacity_kn;
    let utilization = vu_kn / v_capacity_kn;

    // Required steel if current is insufficient
    let asf_required_mm2 = if !passed {
        let v_deficit = vu_kn - v_concrete_kn;
        if v_deficit > 0.0 {
            (v_deficit * 1000.0) / (mu * fyd)
        } else {
            0.0
        }
    } else {
        steel_area_crossing_mm2
    };

    let message = if passed {
        format!(
            "✓ Shear friction OK: V_u = {:.2} kN ≤ V_capacity = {:.2} kN (utilization: {:.2})",
            vu_kn, v_capacity_kn, utilization
        )
    } else {
        format!(
            "✗ Shear friction FAIL: V_u = {:.2} kN > V_capacity = {:.2} kN (utilization: {:.2}) — Increase A_sf to {:.0} mm²",
            vu_kn, v_capacity_kn, utilization, asf_required_mm2
        )
    };
    ...
    ShearFrictionResult {
        ...
        tau_u_mpa: tau_u,
        asf_provided_mm2: steel_area_crossing_mm2,
        asf_required_mm2,
        message,
    }
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ShearFrictionResult {
    pub vu_kn: f64,
    pub v_concrete_kn: f64,
    pub v_steel_kn: f64,
    pub v_capacity_kn: f64,
    pub utilization: f64,
    pub passed: bool,
    pub interface_type: String,
    pub friction_coefficient: f64,
    pub tau_u_mpa: f64,
    pub asf_provided_mm2: f64,
    pub asf_required_mm2: f64,
    pub message: String,
}
