...
use rust_api::solver::pdelta::{PDeltaSolver, PDeltaConfig, PDeltaResult, MemberGeometry};
...
impl BurjKhalifaModel {
    ...
    fn new() -> Self {
        ...
        for floor in 0..num_floors {
            ...
            let core_section = core_sections.iter()
                .find(|s| z_mid >= s.height_range.0 && z_mid < s.height_range.1)
                .unwrap_or(&core_sections[0]);
            
            let perimeter_section = perimeter_sections.iter()
                .find(|s| z_mid >= s.height_range.0 && z_mid < s.height_range.1)
...
fn main() {
    ...
    let difference = ((result.max_displacement - published_displacement) / published_displacement * 100.0).abs();
    
    println!("Validation:");
    println!("  Published max displacement: {:.3} m", published_displacement);
    println!("  Computed max displacement: {:.3} m", result.max_displacement);
