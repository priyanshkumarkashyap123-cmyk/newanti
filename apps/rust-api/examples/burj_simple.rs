...
use beamlab_rust_api::solver::pdelta::{MemberGeometry, PDeltaConfig, PDeltaSolver};
...
fn main() {
    ...
    if top_displacement < drift_limit {
        println!(
            "  ✅ Drift check: PASS ({:.1}% of limit)",
            top_displacement / drift_limit * 100.0
        );
    } else {
        println!(
            "  ❌ Drift check: FAIL ({:.1}% of limit)",
            top_displacement / drift_limit * 100.0
        );
    }
    ...
    let difference =
        ((top_displacement - published_displacement) / published_displacement * 100.0).abs();

    println!("Comparison to Published Data:");
    println!(
        "  Published top displacement: {:.3} m",
