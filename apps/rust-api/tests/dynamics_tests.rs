...
fn test_modal_sdof_oscillator() {
    ...
    assert!(
        (period - 0.6283).abs() < 0.01,
        "Expected T=0.628s, got {}s",
        period
    );
}
...
fn test_modal_2dof_oscillator() {
    ...
    let k = DMatrix::from_row_slice(2, 2, &[200.0, -100.0, -100.0, 200.0]);
    ...
    assert!(
        (omega1 - 10.0).abs() < 0.5,
        "Mode 1: expected ω=10, got {}",
        omega1
    );
    assert!(
        (omega2 - 17.32).abs() < 0.5,
        "Mode 2: expected ω=17.32, got {}",
        omega2
    );

    println!(
        "Mode 1: ω = {:.2} rad/s, T = {:.3} s",
        omega1, result.periods[0]
    );
    println!(
        "Mode 2: ω = {:.2} rad/s, T = {:.3} s",
        omega2, result.periods[1]
    );
}
...
fn test_modal_cantilever_beam() {
    ...
    println!(
        "Cantilever fundamental frequency: {:.2} rad/s",
...
fn test_time_history_sdof_free_vibration() {
    ...
    let result = solver
        .analyze(&k, &m, &force_history, Some(&u0), None)
        .unwrap();
    ...
    assert!(
        max_disp > 0.9 && max_disp < 1.1,
        "Expected max ~1.0, got {}",
        max_disp
    );
...
fn test_time_history_sdof_forced_vibration() {
    ...
    println!(
        "Forced vibration max displacement: {:.3} m",
        result.max_displacements[0]
    );
}
...
fn test_time_history_with_rayleigh_damping() {
    ...
    assert!(
        final_disp < 0.5,
        "Expected decay, final displacement: {}",
        final_disp
    );
...
fn test_integration_method_newmark_average_acceleration() {
    ...
    let method = IntegrationMethod::Newmark {
        beta: 0.25,
        gamma: 0.5,
    };
...
fn test_damping_model_rayleigh() {
    let damping = DampingModel::Rayleigh {
        alpha: 0.1,
        beta: 0.01,
    };
    match damping {
...
fn test_time_history_performance() {
    ...
    let force_history: Vec<DVector<f64>> = (0..n_steps).map(|_| DVector::zeros(n)).collect();
    ...
    assert!(
        elapsed < 500,
        "Analysis too slow: {} ms (target <500ms for 10 DOF, 1000 steps)",
        elapsed
    );

    println!(
        "Time-history performance (10 DOF, 1000 steps): {} ms",
        elapsed
    );
}
